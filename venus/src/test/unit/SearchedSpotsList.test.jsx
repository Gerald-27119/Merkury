...
describe("SearchedSpotsList unit tests", () => {
  ...
  test("Should render title", () => {
    expect(screen.getByText(/Spots matching criteria/i)).toBeInTheDocument();
  });
  ...
  describe("Should render spot info elements", () => {
    test("Should render first photo", () => {
      const firstPhoto = screen.getByAltText(`${mockData.content[0].name}Img`);
      expect(firstPhoto).toBeInTheDocument();
      expect(firstPhoto).toHaveRole("img");
    });
    test("Should render spot name", () => {
      expect(screen.getByText(mockData.content[0].name)).toBeInTheDocument();
    });
    test("Should render spot rating", () => {
      const ratingWrapper = screen.getByTestId("searched-spot-rating");

      const fullStars = ratingWrapper.querySelectorAll(".ant-rate-star-full");
      const halfStars = ratingWrapper.querySelectorAll(".ant-rate-star-half");
