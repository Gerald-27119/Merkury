export const SPOTS = [
  {
    id: 1,
    name: "Pomnik konny Jana III Sobieskiego",
    description:
      "Brązowy posąg XVII-wiecznego polskiego króla Jana III Sobieskiego na koniu usytuowany na małym placu.",
    rating: 2.05,
    location: "Gdańsk",
    categories: ["city", "photos"],
    photos: [
      {
        id: 1,
        img: "https://upload.wikimedia.org/wikipedia/commons/8/8f/John_III_Sobieski_Monument_in_Gda%C5%84sk_2669.JPG",
        title: "pomnik",
        description: "pomnik",
      },
      {
        id: 2,
        img: "https://gaps.gda.pl/wp-content/uploads/2021/09/SM_065-1-1024x681.jpg",
        title: "pomnik",
        description: "pomnik",
      },
      {
        id: 3,
        img: "https://i1.nocimg.pl/d6/416/158-gdansk-pomnik-jana-iii-sobieskiego.jpg",
        title: "pomnik",
        description: "pomnik",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.352223,
      lon: 18.647865,
    },
    comments: [
      {
        text: "Świetne miejsce, warto odwiedzić!",
        rating: 5.0,
        publishDate: "2024-06-01T10:15:00",
        author: "user",
      },
      {
        text: "Było fajnie, choć spodziewałem się więcej.",
        rating: 4.0,
        publishDate: "2024-06-02T14:30:00",
        author: "user",
      },
      {
        text: "Comment0: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 1.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment1: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 2.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment2: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 3.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment3: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 4.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment4: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 0.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment5: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 1.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment6: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 2.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment7: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 3.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment8: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 4.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      {
        text: "Comment9: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        rating: 0.0,
        publishDate: "2024-06-20T12:00:00",
        author: "user",
      },
      /* ... Additional generated comments (Comment10 to Comment99) follow the same pattern ... */
    ],
  },
  {
    id: 2,
    name: "Skwer Czesława Niemena",
    description: "Mały park z ławkami i pomnikiem Czesława Niemena.",
    rating: 4.0,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 4,
        img: "https://lh5.googleusercontent.com/p/AF1QipONWLJ_OEhtiG9pHEZbmmBNZiaQuVG82uROLoj9=w408-h306-k-no",
        title: "skwer",
        description: "skwer",
      },
      {
        id: 5,
        img: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/69/Gda%C5%84sk_Skwer_Czes%C5%82awa_Niemena.JPG/2560px-Gda%C5%84sk_Skwer_Czes%C5%82awa_Niemena.JPG",
        title: "skwer",
        description: "skwer",
      },
      {
        id: 6,
        img: "https://s-trojmiasto.pl/zdj/c/n/9/2986/750x600/2986010-Zielen-na-terenie-skweru-Niemena.webp",
        title: "skwer",
        description: "skwer",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.352541,
      lon: 18.643992,
    },
    comments: [
      {
        text: "Idealne miejsce na relaks.",
        rating: 5.0,
        publishDate: "2024-06-03T09:45:00",
        author: "user",
      },
      {
        text: "Widoki niezłe, ale tłoczno i głośno.",
        rating: 3.0,
        publishDate: "2024-06-04T16:20:00",
        author: "user",
      },
    ],
  },
  {
    id: 3,
    name: "Park Wałowy",
    description: "Mały park z ławkami",
    rating: 4.25,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 7,
        img: "https://s-trojmiasto.pl/zdj/c/n/9/2922/1920x0/2922075-Obecnie-to-nieco-zapomniany-teren-ktory-czeka-na-rewitalizacje.webp",
        title: "park wałowy",
        description: "park wałowy",
      },
      {
        id: 8,
        img: "https://streetviewpixels-pa.googleapis.com/v1/thumbnail?panoid=gmCXVf6Sftf-V9WiIuwVUg&cb_client=search.gws-prod.gps&w=408&h=240&yaw=232.89917&pitch=0&thumbfov=100",
        title: "park wałowy",
        description: "park wałowy",
      },
      {
        id: 9,
        img: "https://s-trojmiasto.pl/zdj/c/n/9/2331/1500x0/2331938-Obecnie-plac-Walowy-to-zaniedbane-miejsce.webp",
        title: "park wałowy",
        description: "park wałowy",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.34259835347914,
      lon: 18.646824493647234,
    },
    comments: [
      {
        text: "Czysto, spokojnie i klimatycznie.",
        rating: 5.0,
        publishDate: "2024-06-05T08:10:00",
        author: "user",
      },
      {
        text: "Trochę zbyt mało atrakcji jak dla mnie.",
        rating: 3.5,
        publishDate: "2024-06-06T18:55:00",
        author: "user",
      },
    ],
  },
  {
    id: 4,
    name: "Park",
    description: "Mały park",
    rating: 3.75,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 10,
        img: "https://lh5.googleusercontent.com/p/AF1QipMBnWPy-4MYt0WQbIC8HafWEPN7GwTl6I7XrWdN=w408-h544-k-no",
        title: "park",
        description: "park",
      },
      {
        id: 11,
        img: "https://lh5.googleusercontent.com/p/AF1QipMT3elr7EwlwKnqxpmGfoQ2lOlYxR213NagDzOK=w408-h725-k-no",
        title: "park",
        description: "park",
      },
      {
        id: 12,
        img: "https://gdansk.gedanopedia.pl/images/6/63/Pomnik_kwiat.JPG",
        title: "park",
        description: "park",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.35165940763592,
      lon: 18.648793774997493,
    },
    comments: [
      {
        text: "Miejsce warte odwiedzenia, polecam.",
        rating: 4.5,
        publishDate: "2024-06-07T11:40:00",
        author: "user",
      },
      {
        text: "Atmosfera w porządku, ale spodziewałem się więcej zieleni.",
        rating: 3.0,
        publishDate: "2024-06-08T13:25:00",
        author: "user",
      },
    ],
  },
  {
    id: 5,
    name: "Jar Wilanowski",
    description: "Zielona strefa z jeziorem",
    rating: 4.5,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 13,
        img: "https://d-art.ppstatic.pl/kadry/k/r/75/0b/59b1497fa1405_o_original.jpg",
        title: "jar",
        description: "jar",
      },
      {
        id: 14,
        img: "https://s-trojmiasto.pl/zdj/c/n/9/2189/1500x0/2189954-We-wschodniej-czesci-jaru-powstaly-m-in-pola-do-minigolfa-stoly-pingpongowe-oraz-plac-sasiedzki.webp",
        title: "jar",
        description: "jar",
      },
      {
        id: 15,
        img: "https://foto.cloudgdansk.pl/foto/Jar_Wilanowski_81897_1280px.JPG",
        title: "jar",
        description: "jar",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.33273052789799,
      lon: 18.607928515378298,
    },
    comments: [
      {
        text: "Rewelacyjne miejsce na wycieczkę!",
        rating: 5.0,
        publishDate: "2024-06-09T07:50:00",
        author: "user",
      },
      {
        text: "Dobre miejsce, ale trochę za drogo jak na jakość.",
        rating: 4.0,
        publishDate: "2024-06-10T20:15:00",
        author: "user",
      },
    ],
  },
  {
    id: 6,
    name: "Plac imienia Dariusza Kobzdeja",
    description:
      "Mały, zadbany plac z ławeczkami i zielenią. Znajduje się on z jednej strony w pobliżu pomnika Jana III Sobieskiego, a z drugiej strony w pobliżu Hali Targowej.",
    rating: 4.5,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 16,
        img: "https://gaps.gda.pl/wp-content/uploads/2021/08/SM_031-1-1024x681.jpg",
        title: "plac kobzdeja",
        description: "plac kobzdeja",
      },
      {
        id: 17,
        img: "https://upload.wikimedia.org/wikipedia/commons/a/ab/Gdansk-plac_Kobzdeja-obelisk-Herbert.jpg",
        title: "plac kobzdeja",
        description: "plac kobzdeja",
      },
      {
        id: 18,
        img: "https://upload.wikimedia.org/wikipedia/commons/c/cd/Gda%C5%84sk_plac_Kobzdeja.JPG",
        title: "plac kobzdeja",
        description: "plac kobzdeja",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.35200732546214,
      lon: 18.65021445366572,
    },
    comments: [
      {
        text: "Wspaniałe widoki, aż chce się wracać.",
        rating: 5.0,
        publishDate: "2024-06-11T15:30:00",
        author: "user",
      },
      {
        text: "Było przyjemnie, choć obsługa mogłaby być milsza.",
        rating: 4.0,
        publishDate: "2024-06-12T19:05:00",
        author: "user",
      },
    ],
  },
  {
    id: 7,
    name: "Plac Zabaw na Wroniej Górce",
    description: "Plac zabaw",
    rating: 4.25,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 19,
        img: "https://lh5.googleusercontent.com/p/AF1QipPV2HWroKJF1MEQ542bKpmIlLGNj42dNJM2N2w=w408-h306-k-no",
        title: "wrona",
        description: "wrona",
      },
      {
        id: 20,
        img: "https://files.cloudgdansk.pl/files/objects/thumb/10155_275.jpg",
        title: "wrona",
        description: "wrona",
      },
      {
        id: 21,
        img: "https://ibedeker.pl/wp-content/gallery/wronia-gorka/img_2006.jpg",
        title: "wrona",
        description: "wrona",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.358542876913596,
      lon: 18.63203900299652,
    },
    comments: [
      {
        text: "Bardzo ciekawe miejsce z historią.",
        rating: 5.0,
        publishDate: "2024-06-13T12:10:00",
        author: "user",
      },
      {
        text: "Miejsce okej, ale parking był problematyczny.",
        rating: 3.5,
        publishDate: "2024-06-14T14:50:00",
        author: "user",
      },
    ],
  },
  {
    id: 8,
    name: "Plaża stogi",
    description: "Szeroka piaszczysta plaża.",
    rating: 4.25,
    location: "Gdańsk",
    categories: ["city", "beach", "photos"],
    photos: [
      {
        id: 22,
        img: "https://lh5.googleusercontent.com/p/AF1QipMF6_xAfFIKoVnLYaW5agOblgtiDZUomDBDUuCt=w408-h306-k-no",
        title: "plaża",
        description: "plaża",
      },
      {
        id: 23,
        img: "https://stogi.info.pl/wp-content/uploads/2024/02/Kompleksowy_przewodnik_po_plazy_Jelitkowo_atrakcje_historia_i_ciekawostki_2-1080x675.jpg",
        title: "plaża",
        description: "plaża",
      },
      {
        id: 24,
        img: "https://i.ytimg.com/vi/hs9TdD8TZd4/maxresdefault.jpg",
        title: "plaża",
        description: "plaża",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.38058274498329,
      lon: 18.71852472241705,
    },
    comments: [
      {
        text: "Czyste i dobrze zorganizowane miejsce.",
        rating: 4.5,
        publishDate: "2024-06-15T09:00:00",
        author: "user",
      },
      {
        text: "Naprawdę wyjątkowe miejsce, choć trochę za dużo ludzi.",
        rating: 4.0,
        publishDate: "2024-06-16T18:45:00",
        author: "user",
      },
    ],
  },
  {
    id: 9,
    name: "Park Oruński im. Emilii Hoene",
    description:
      "Park Oruński należy, obok Parku Oliwskiego, należy do najcenniejszych zachowanych dawnych gdańskich parków.",
    rating: 4.25,
    location: "Gdańsk",
    categories: ["city", "park", "photos"],
    photos: [
      {
        id: 25,
        img: "https://d-art.ppstatic.pl/kadry/k/r/9c/65/5df1faf14a73e_o_large.jpg",
        title: "orunia",
        description: "orunia",
      },
      {
        id: 26,
        img: "https://lh5.googleusercontent.com/p/AF1QipOL4nY6WdI-icmazsrcesbFB9zkd1WSsgQFSnah=w408-h306-k-no",
        title: "orunia",
        description: "orunia",
      },
      {
        id: 27,
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTRR-F1umTowNMlCuAQNZwmd6p4Xet-hQg0bQ&s",
        title: "orunia",
        description: "orunia",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.3230684167437,
      lon: 18.629913718953556,
    },
    comments: [
      {
        text: "Super miejsce na rodzinny wypad.",
        rating: 5.0,
        publishDate: "2024-06-17T08:30:00",
        author: "user",
      },
      {
        text: "Nie najgorsze, ale brakowało mi większych atrakcji.",
        rating: 3.5,
        publishDate: "2024-06-18T21:10:00",
        author: "user",
      },
    ],
  },
  {
    id: 10,
    name: "Park Street Workout",
    description:
      "Park, który oryginalnie był cmentarzem protestanckim, należącym dawniej do kościoła przy placu Oruńskim.",
    rating: 4.75,
    location: "Gdańsk",
    categories: ["city", "park", "FPV", "photos"],
    photos: [
      {
        id: 28,
        img: "https://lh5.googleusercontent.com/p/AF1QipPfl720UMdhBi_3zoXU2qJJ2QJABydhihYSO3WX=w408-h306-k-no",
        title: "workout",
        description: "workout",
      },
      {
        id: 29,
        img: "https://files.cloudgdansk.pl/files/objects/thumb/10276_275.jpg",
        title: "workout",
        description: "workout",
      },
    ],
    weatherApiCallCoords: {
      lat: 54.32202428469449,
      lon: 18.63395637969597,
    },
    comments: [
      {
        text: "Miejsce godne polecenia, świetna organizacja.",
        rating: 5.0,
        publishDate: "2024-06-19T10:20:00",
        author: "user",
      },
      {
        text: "Podobało mi się, choć były drobne niedociągnięcia.",
        rating: 4.5,
        publishDate: "2024-06-20T17:35:00",
        author: "user",
      },
    ],
  },
];
