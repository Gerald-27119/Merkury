...
\newcommand{\woczatcard}[5]{%
    ...
    \endfirsthead
%TODO zrobilem ze karty sie aldnie lamia, pytanie czy chcemy to ? wtedy zrobic to wszedzie?
    \hline
    \rowcolor{lightgray}\multicolumn{4}{|c|}
...
{M}
{
    ...
    \woczinput{Treść wiadomości w postaci tekstowej lub pliku (np. zdjęcia).}

    \woczpre{Użytkownik jest zalogowany i posiada dostęp do danego czatu.}

    \woczpost{Nowo wysłana wiadomość staje się częścią historii danego czatu i jest
    dostępna dla wszystkich uczestników konwersacji.}
