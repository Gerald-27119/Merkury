...
\begin{itemize}
    \item \textbf{Strona główna} – strona główna forum prezentująca listę postów,
    \item \textbf{Szczegóły posta} – widok zawierający pełną treść posta wraz z obrazami i komentarzami,
    \item \textbf{Obserwowane wątki} – lista postów obserwowanych przez użytkownika,
    \item \textbf{Historia postów} – lista ostatnio przeglądanych postów oraz postów ocenionych przez użytkownika.
\end{itemize}

Interfejs forum został zaprojektowany w ciemnym trybie kolorystycznym.

\subsubsection{Strona główna}
Widok główny forum (rys. \ref{img:forum-view}) składa się z trzech głównych obszarów funkcjonalnych.
...
Element umożliwia wyszukiwanie postów na forum.
Po rozwinięciu paska wyszukiwania (rys. \ref{img:searchbar_extended}) dostępne są dodatkowe filtry:
\begin{itemize}
    \item kategorie,
    \item tagi,
...
Widok ten (rys. \ref{img:post-details}) prezentuje pełną treść wraz z osadzonymi obrazami.
...
\subsubsection{Obserwowane wątki}
Widok ten prezentuje listę postów obserwowanych (rys. \ref{img:followed-threads}) przez użytkownika.
...
\subsubsection{Historia postów}
Podstrona \textit{Historia postów} umożliwia przeglądanie wątków (rys. \ref{img:post-history}), z którymi użytkownik wchodził w interakcje.
Dostęp do podstrony realizowany jest poprzez wybór opcji \textit{Posts history} w panelu bocznym.
