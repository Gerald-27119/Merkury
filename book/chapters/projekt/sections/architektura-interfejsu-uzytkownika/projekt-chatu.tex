...
Widoczna jest opcja umożliwiająca dodanie załączników, wstawienie GIF oraz użycie emotikon.
...
okno modalne z nagłówkiem \textit{Pinned messages}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{\chatfig{OptionsMessageHover3DotsMenu}}
...
Rysunek \ref{fig:chat:own-message-hover} przedstawia stan interfejsu po
najechaniu kursorem na wiadomość wysłaną przez obecnego użytkownika.

Nad wiadomością widoczny jest pasek akcji w formie poziomego rzędu ikon.
Menu prezentuje listę operacji:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{\chatfig{MyMessageHover3DotsMenu}}
    \caption{Menu „więcej opcji” dla wiadomości własnej.}
    \label{fig:chat:own-message-more-menu}
\end{figure}

Rysunek \ref{fig:chat:own-message-more-menu} przedstawia stan interfejsu po
rozwinięciu menu „więcej opcji” (ikona trzech kropek) dla wiadomości wysłanej
przez zalogowanego użytkownika. Nakładka zawiera operacje takie jak
\textit{Pin message} oraz \textit{Copy text}, a także akcje wyróżnione kolorem czerwonym:
\textit{Report} i \textit{Delete}. Umieszczenie menu bezpośrednio przy wiadomości
umożliwia wykonanie operacji bez opuszczania bieżącego widoku rozmowy.
...
Rysunek \ref{fig:chat:media-grid} przedstawia widok, w którym w obrębie aktywnej rozmowy
udostępniono podgląd materiałów wizualnych powiązanych z konwersacją.
W centralnej części ekranu wyświetlono siatkę miniatur, obejmującą co najmniej dwa elementy.
Jedna z miniatur przedstawia obraz, natomiast druga zawiera nałożony symbol odtwarzania,
co wizualnie odróżnia ją od zwykłej grafiki (materiał wideo).
Widok pozostaje osadzony w kontekście modułu czatu: po lewej stronie nadal widoczna jest lista rozmów,
a w górnym pasku zachowano elementy sterujące związane z konwersacją oraz wyróżnioną ikonę sekcji multimediów.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{\chatfig{ImageMenu}}
...
Dodatkowo widoczna jest opcja usunięcia wyróżniona kolorem czerwonym, co wskazuje na działanie o charakterze destrukcyjnym.
Rozmieszczenie menu w pobliżu wskazanego elementu umożliwia wykonanie operacji bez opuszczania bieżącego widoku rozmowy.
...
do której nawiązano. Podgląd zawiera podstawowe informacje (np. awatar oraz nazwę autora i fragment treści),
dzięki czemu użytkownik może jednoznacznie powiązać odpowiedź z konkretnym wpisem w historii rozmowy.
...
Rysunek \ref{fig:chat:create-modal} przedstawia widok tworzenia nowej rozmowy w formie okna modalnego.
...
a także elementy sterujące analogiczne do rozmów prywatnych, w tym wyszukiwanie oraz sekcja multimediów.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{\chatfig{GroupChatMenu}}
...
Nakładka zawiera operacje związane z konfiguracją rozmowy, takie jak zmiana obrazu grupy lub jej nazwy.
Każda pozycja jest opatrzona ikoną, co ułatwia szybkie rozpoznanie dostępnych działań bez potrzeby opuszczania widoku rozmowy.
...
umożliwiający szybkie wybranie osoby do oznaczenia. Po prawej stronie ekranu zachowano listę członków grupy.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{\chatfig{ActiveGroupChatPageGroupMemberOptions}}
