...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/mapa/mapa_lokalizacja_uzytkownika}
...
Na liście każdy element zawiera nazwę \glslink{spot}{spota}, jego ocenę wraz z ich liczbą, listę tagów oraz zdjęcie.
Kliknięcie jednego z wyników powoduje przybliżenie widoku mapy na jego lokalizację.
...
według minimalnej oceny \glslink{spot}{spota} (rys. \ref{fig:mapa-search-filtrowanie-ocena}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/mapa/search_area_filtered_spots}
...
Każdy element na liście prezentuje nazwę \glslink{spot}{spota}, jego ocenę wraz z ich liczbą, listę tagów oraz zdjęcie.
Kliknięcie na wybraną pozycję powoduje przybliżenie widoku mapy na jej lokalizację.
...
Prezentowane wyniki można sortować po ocenie lub liczbie ocen, w obu przypadkach dostępne są warianty: malejąco i rosnąco.
...
Galeria multimediów pozwala na ich przeglądanie za pomocą umieszczonych na jej bokach strzałek.
Kliknięcie zdjęcia powoduje wyświetlenie go w dużej wersji galerii (por. sekcja~\ref{subsec:galeria-multimediow}), natomiast w przypadku filmu \textendash \space jego odtworzenie.

Znajdujące się poniżej przyciski akcji pozwalają użytkownikowi na wykonanie czynności związanych z wybranym \glslink{spot}{spotem}:
\begin{itemize}
    ...
    pamięci podręcznej systemu o czym informuje odpowiedni komunikat; wklejenie go w pasek przeglądarki
    ...
    Jeżeli żadne multimedia nie zostaną dołączone, wysłanie formularza będzie niemożliwe i zostanie wyświetlona informacja o błędzie.
    ...
    O pomyślnym przebiegu operacji informuje powiadomienie, a nowo dodane elementy widoczne są w galerii multimediów \glslink{spot}{spota}.
\end{itemize}
...
a w przeciwnym wypadku otwierany jest formularz zawierający pole do wystawienia oceny, dodania multimediów oraz tekstu (rys. \ref{fig:spot-add-comment-form}).
...
Jeżeli pola zostaną błędnie wypełnione zaprezentowany zostanie odpowiedni komunikat, a wysłanie formularza będzie niemożliwe.
O pomyślnym przebiegu operacji informuje powiadomienie, a nowy komentarz jest dodany do wybranego \glslink{spot}{spota}.
...
Naciśnięcie jednego z elementów listy powoduje otworzenie go w galerii multimediów \glslink{spot}{spota} (por. sekcja~\ref{subsec:galeria-multimediow}).
...
Po jego kliknięciu pobierana jest strona druga (rys. \ref{fig:spot-comment-more-loaded}), a pozostałe zgodnie z mechanizmem \glslink{infinite-scroll}{infinite scroll}.
...
Po kliknięciu wybranego kafelka możliwa jest również zmiana wysokości, dla której wyświetlane są dane (rys. \ref{fig:spot-detailed-weather-wind-speeds-height}).
...
W dolnej części panelu znajduje się wykres prezentujący prognozowaną pogodę na trzy kolejne dni z dokładnością do godziny.
...
Galeria multimediów \glslink{spot}{spota} prezentuje jego wszystkie zdjęcia i filmy (rys. \ref{fig:spot-expanded-media-gallery}).
...
\textendash \space zostanie ona wyróżniona poprzez zmianę wyglądu (rys. \ref{fig:spot-expanded-media-gallery-change}).
...
Listę można sortować po dacie dodania (od najnowszego lub najstarszego) i według liczby polubień (od największej).
...
Pod powiększonym elementem znajduje się panel pozwalający użytkownikowi na wykonanie następujących akcji:
\begin{itemize}
    \item powiększenie zdjęcia lub filmu na cały ekran;
    \item pobranie pliku;
...
Powyżej umieszczone są przyciski do zamknięcia galerii oraz udostępnienia zdjęcia lub filmów.
Komunikat informuje o skopiowaniu linku do pamięci podręcznej systemu.
