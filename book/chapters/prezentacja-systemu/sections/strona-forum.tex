...
Forum jest dostępne od momentu wejścia na stronę również dla użytkowników niezalogowanych, jednak wyłącznie w trybie odczytu.
Wszelkie próby interakcji z treściami wymagają poprawnego zalogowania.
...
Interfejs forum został zaprojektowany w wersji jasnej oraz ciemnej, przy czym większość zdjęć wyświetlana jest w trybie jasnym.

\subsection{Home Page}
\label{subsec:home-page}

Strona główna forum składa się z trzech głównych obszarów funkcjonalnych (rys. \ref{img:home-page}) (rys. \ref{img:home-page-dark}).
...
Próba wykonania tych operacji przez użytkownika niezalogowanego skutkuje wyświetleniem odpowiedniego komunikatu (rys. \ref{img:follow-post-warning}) (rys. \ref{img:report-post-warning}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/forum/follow_post_warning}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/forum/follow_post_warning}
    \caption{Komunikat informujący o konieczności zalogowania w celu dodania posta do obserwowanych}
    \label{img:follow-post-warning}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/forum/report_post_warning}
    \caption{Komunikat informujący o konieczności zalogowania w celu zgłoszenia posta}
    \label{img:report-post-warning}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{content} -- treść posta tworzona za pomocą edytora typu \textit{rich text}
\end{itemize}
...
W przypadku braku wymaganych informacji pojawia się informacja pod polem nie spełniającym wymagań (rys. \ref{img:create-post-validation}).
...
\begin{itemize}
    ...
    \item liczbę komentarzy
    \item przycisk \textit{share} umożliwiający skopiowanie adresu URL posta
...
Naciśnięcie przycisku \textit{Add comment} spowoduje otwarcie formularza dodania komentarza (rys. \ref{img:add-comment}), natomiast w przypadku użytkownika niezalogowanego wyświetlany jest odpowiedni komunikat informujący o konieczności zalogowania (rys. \ref{img:add-comment-warning}).
W formularzu zastosowano walidację danych.
W przypadku braku wymaganych informacji pojawia się informacja pod polem nie spełniającym wymagań (rys. \ref{img:comment-validation}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/prezentacja-systemu/forum/add_comment_warning}
    \caption{Komunikat informujący o konieczności zalogowania w celu dodania komentarza}
    \label{img:add-comment-warning}
\end{figure}

Pod kafelkiem posta wyświetlana jest lista komentarzy (rys. \ref{img:comments-replies-closed}).
Komentarze mają formę wizualnie zbliżoną do posta, z wyłączeniem funkcji udostępniania.
Każdy komentarz zawiera przycisk \textit{reply} umożliwiający dodanie odpowiedzi oraz menu kontekstowe pozwalające autorowi na edycję lub usunięcie treści.
...
W przypadku komentarzy posiadających odpowiedzi wyświetlana jest ikona strzałki wraz z liczbą odpowiedzi, umożliwiająca rozwinięcie ich listy (rys. \ref{img:comments-replies-open}).
...
Dostęp do podstrony realizowany jest poprzez wybór odpowiedniej sekcji w panelu bocznym po poprawnym zalogowaniu.
...
Przejście do niej odbywa się poprzez wybór odpowiedniej sekcji w panelu bocznym.
