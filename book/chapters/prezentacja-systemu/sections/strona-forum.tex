...
\begin{itemize}
    \item Strona główna
    \item Szczegóły posta
    \item Lista wszystkich kategorii
    \item Lista wszystkich tagów
    \item Obserwowane wątki
    \item Regulamin forum
\end{itemize}

\subsection{Strona główna}
\label{subsec:home-page}

Widok strony głównej forum (rys. \ref{img:home-page}) składa się z trzech podstawowych obszarów funkcjonalnych:
\begin{itemize}
...
Dostępne opcje to:
\begin{itemize}
    \item \textit{Newest} (najnowsze)
    \item \textit{Oldest} (najstarsze)
    \item \textit{Most Viewed} (najczęściej wyświetlane)
    \item \textit{Least Viewed} (najrzadziej wyświetlane)
    \item \textit{Most Commented} (najczęściej komentowane)
    \item \textit{Least Commented} (najrzadziej komentowane)
\end{itemize}
...
\paragraph{Pasek wyszukiwania}

Pasek wyszukiwania (rys. \ref{img:search-bar}) umożliwia wyszukiwanie postów na forum.
Po jego rozwinięciu dostępne są dodatkowe filtry, w tym:
\begin{itemize}
    \item kategoria
    \item tagi
    \item zakres dat (od, do)
    \item autor
\end{itemize}
...
\paragraph{Popularne posty}

Sekcja \textit{Trending Posts} (rys. \ref{img:right-panel}) prezentuje listę trzech najpopularniejszych postów z okresu ostatniego miesiąca.
...
Poniżej wyświetlane są kategoria i tagi, następnie tytuł oraz pełna treść.
...
\subsection{Lista wszystkich kategorii}

Podstrona prezentuje alfabetyczną listę wszystkich kategorii postów (rys. \ref{img:categories}).
...
\subsection{Lista wszystkich tagów}

Podstrona wyświetla alfabetyczną listę wszystkich tagów (rys. \ref{img:tags}).
