...
{%
    \begin{tabitemize}
        \item \textbf{body}: ChatDto, zawiera m.in.:
        \begin{itemize}
            \item \textbf{id}: Long (identyfikator czatu)
            \item \textbf{name}: String (nazwa czatu – np. nazwa grupy lub rozmówcy)
            \item \textbf{lastMessage}: ChatMessageDto (ostatnia wiadomość w czacie;
            struktura jak w EP38)
            \item \textbf{imgUrl}: String (URL obrazka/avatara czatu)
            \item \textbf{messages}: List<ChatMessageDto> (lista wiadomości zwróconych
            razem z czatem)
            \item \textbf{chatType}: ChatType (typ czatu, np. \texttt{PRIVATE}, \texttt{GROUP})
