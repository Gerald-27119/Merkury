...
Do integracji ze środowiskiem \glslink{spring-boot}{Spring Boot} użyto gotowego mechanizmu \glslink{spring-cache}{Spring cache}.
Konfiguracja menedżera \glslink{cache}{cache} została oparta o \texttt{RedisCacheManager}, a dla poszczególnych przestrzeni
\glslink{cache}{cache} zdefiniowano niezależne czasy życia wpisów (\gls{ttl}).
...
\glslink{cache}{Cachowanie} danych realizowane jest \glslink{annotation}{adnotacją} \texttt{@Cacheable}, która pozwala
...
Wartości \gls{ttl} dobrano na podstawie charakteru danych:
dla wyników wyszukiwania zastosowano krótsze czasy (częsta zmienność i większe ryzyko nieaktualności),
natomiast dla danych pogodowych lub \glslink{gif}{gifów}, które zmieniają się wolniej, ustawiono dłuższe czasy przechowywania.
