...
Rozwiązanie to nawiązuje do \glslink{wzorzec}{wzorca} \textit{Chain of Responsibility} opisanego w rozdziale ~\ref{subsubsec:chain-of-responsibility}.
...
\begin{itemize}
    ...
    \texttt{UrlsProperties}, \newline \texttt{JwtProperties}.
...
dzięki czemu pola klasy mogą być mapowane z pliku konfiguracyjnego aplikacji \newline \texttt{application.properties}.
...
\newline \texttt{@ConfigurationProperties(prefix = "jwt")},
...
reprezentujące klucz kryptograficzny używany do podpisywania i weryfikacji \glslink{token}{tokenów} \glslink{jwt}{JWT}.
...
\texttt{tokenExpiration} oraz \newline \texttt{tokenCookieExpiration} definiują czas ważności \glslink{token}{tokena} i \glslink{http-only-cookie}{ciasteczka} przechowującego go,
...
\begin{itemize}
    ...
    \newline (\texttt{UrlsProperties} oraz \texttt{CustomUserDetailsService}).
\end{itemize}
...
Bean \texttt{publicSecurityFilterChain} \newline (z \texttt{@Order(1)}) obsługuje wyłącznie żądania
...
\glslink{bean}{Bean} \texttt{privateSecurityFilterChain} \newline (z \texttt{@Order(2)}) dotyczy wszystkich pozostałych żądań,
czyli dopasowanych do \newline \texttt{privatePathsMatcher}. Najważniejsze elementy:
\begin{itemize}
    ...
    \item \textbf{Filtr \glslink{jwt}{JWT} wpięty w łańcuch}: \newline \texttt{addFilterBefore(jwtAuthFilter, LogoutFilter.class)}
...
Na końcu zdefiniowano \texttt{FilterRegistrationBean<JwtAuthFilter>} z \newline \texttt{setEnabled(false)}.
Celem jest wyłączenie automatycznej rejestracji filtra przez warstwę servletową,
ponieważ filtr \glslink{jwt}{JWT} jest dodawany ręcznie do właściwego \newline \texttt{SecurityFilterChain}.
...
\begin{itemize}
    ...
    \item \texttt{JwtService} -- udostępnia interfejs do odświeżenia \glslink{token}{tokenu} z poziomu logiki aplikacyjnej.
\end{itemize}
...
walidację podpisu, parsowanie danych z \glslink{token}{tokenu}, weryfikację czasu ważności oraz obsługę \glslink{http-only-cookie}{ciasteczka HTTP only}.

Metoda \texttt{validateToken(...)} odpowiada za sprawdzenie, czy \glslink{token}{token} istnieje i czy posiada poprawny podpis.
...
Obsłużono również scenariusz \glslink{token}{tokenu} wygasłego \newline (\texttt{ExpiredJwtException}),
...
\texttt{addTokenToCookie(...)} (zapis) oraz \newline \texttt{getJWTFromCookie(...)} (odczyt).
Ciasteczko jest ustawiane jako \texttt{HttpOnly} i ma nazwę określoną w \newline \texttt{JwtProperties.tokenName}.
...
Klasa \texttt{JwtGenerator} odpowiada za generowanie \glslink{token}{tokenów} \glslink{jwt}{JWT}.
Wygenerowany \glslink{token}{token} zawiera podstawowe informacje wymagane do identyfikacji użytkownika:
\begin{itemize}
    \item \textbf{subject} -- nazwa użytkownika, pobierana z \texttt{SecurityContextHolder},
    \item \textbf{issuedAt} -- czas wystawienia \glslink{token}{tokenu},
    \item \textbf{expiration} -- czas wygaśnięcia obliczany na podstawie \newline \texttt{JwtProperties.tokenExpiration}.
\end{itemize}
...
Poprawny podpis pozwala serwerowi potwierdzić autentyczność \glslink{token}{tokenu} oraz wykryć jakąkolwiek modyfikację jego zawartości.
...
\begin{itemize}
    \item odczytuje \glslink{token}{token} z \glslink{http-only-cookie}{ciasteczka} poprzez \texttt{JwtManager.getJWTFromCookie(...)}.
    \item weryfikuje \glslink{token}{token} (\texttt{JwtManager.validateToken(...)}); w przypadku błędu zwraca odpowiedź \texttt{401 Unauthorized}
    w formacie \gls{json} i nie przekazuje żądania dalej.
    \item odczytuje identyfikator użytkownika (subject) z \glslink{token}{tokenu} i ładuje dane użytkownika
...
jeżeli \glslink{token}{token} jest nadal ważny, ale do jego wygaśnięcia pozostał czas krótszy niż jeden dzień
\newline (\texttt{JwtProperties.oneDayInMs}), generowany jest nowy \glslink{token}{token} i zapisywany w \glslink{http-only-cookie}{ciasteczku}.
...
Metoda sprawdza, czy w \texttt{SecurityContextHolder} istnieje \newline \texttt{Authentication},
...
o wskazanym identyfikatorze. Metoda \newline \texttt{isUserAChatMember(...)}:
\begin{itemize}
