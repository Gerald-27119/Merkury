...
Klasa \texttt{JwtProperties} jest klasą konfiguracyjną mapowaną przez Spring Boot z użyciem
...
Pozwala to centralnie zarządzać parametrami JWT i nadpisywać je bez modyfikacji kodu.
...
\texttt{tokenExpiration} oraz \texttt{tokenCookieExpiration} definiują czas ważności tokenu i ciasteczka przechowującego token,
...
\begin{itemize}
    ...
    \item \texttt{privatePathsMatcher} -- matcher wskazujący ścieżki wymagające uwierzytelnienia (negacja matchera publicznego (\texttt{NegatedRequestMatcher})).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Konfiguracja CORS}: dopuszcza komunikację z frontendem na innym porcie niż działa backend.
    \item \textbf{Wyłączenie CSRF}: w przypadku aplikacji klienckiej typu \gls{spa}, która komunikuje się z backendem wyłącznie poprzez \gls{api},
    mechanizm CSRF nie jest wykorzystywany.
    ...
    \item \textbf{Zarządzanie sesją}: \texttt{SessionCreationPolicy.IF\_REQUIRED} tworzy sesję tylko na potrzeby logowania lub rejestracji za pomocą OAuth2.
    ...
    W projekcie nie jest on wykorzystywany, ponieważ uwierzytelnianie realizowane jest przez OAuth2 oraz tokeny JWT.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{allowCredentials = true} -- dopuszcza wysyłanie ciasteczek w zapytaniach,
...
\begin{itemize}
    ...
    \item \texttt{JwtManager} -- weryfikuje token i obsługuje operacje na ciasteczku,
...
walidację podpisu, parsowanie danych z tokenu, weryfikację czasu ważności oraz obsługę ciasteczka.
...
Operacje na ciasteczku są realizowane przez:
...
Podpis umożliwia serwerowi weryfikację, czy token nie został zmodyfikowany.
...
w sposób scentralizowany dla wszystkich endpointów wymagających uwierzytelnienia.

W metodzie \texttt{doFilterInternal(...)} filtr realizuje następujące kroki:
\begin{itemize}
    \item odczytuje token z ciasteczka poprzez \texttt{JwtManager.getJWTFromCookie(...)}.
...
(\texttt{JwtProperties.oneDayInMs}), generowany jest nowy token i zapisywany w ciasteczku.
...
a następnie generuje nowy token i zapisuje go w ciasteczku.
...
Oprócz reguł opartych o ścieżki URL, aplikacja wykorzystuje również autoryzację domenową
