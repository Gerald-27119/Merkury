...
Opis przygotowano w oparciu o dokumentację Spring Security:
\begin{itemize}
...
\begin{itemize}
    ...
    Oznacza to zapewnienie, że żądanie pochodzi od konkretnego, znanego użytkownika.
    \item \textbf{Autoryzacja} -- określenie i przestrzeganie polityki dostępu oraz operacji na zasobach,
    do jakich dostęp ma uwierzytelniony użytkownik. Nawet poprawnie uwierzytelniony klient nie powinien
    mieć możliwości wykonania akcji, do których nie posiada uprawnień.
    \item \textbf{Ochrona przed typowymi atakami} -- zestaw mechanizmów ograniczających ryzyko podatności i nadużyć,
    takich jak brute-force, błędna konfiguracja \gls{cors}, czy wykorzystanie luk w warstwie aplikacyjnej.
    ...
    nie mogą zostać łatwo podejrzane lub zmodyfikowane po drodze. W praktyce sprowadza się to głównie do stosowania HTTPS/TLS
    oraz konsekwentnego egzekwowania polityk bezpieczeństwa w warstwie serwera.
\end{itemize}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/diagram-spring-security-chain}
%     \caption{Schemat przetwarzania żądania w Spring Security (łańcuch filtrów)}
%     \label{fig:spring-security-chain}
% \end{figure}
...
(1) klasy mapujące konfigurację z plików środowiskowych (\texttt{UrlsProperties}, \texttt{JwtProperties}),
(2) klasę przygotowującą bean JWT (\texttt{JwtConfig}),
oraz (3) główną konfigurację security (\texttt{SecurityConfig}),
która definiuje reguły dostępu i buduje łańcuchy filtrów.
...
dzięki czemu pola klasy mogą być mapowane z pliku konfiguracyjnego (np.\ \texttt{application.yml}).
W praktyce oznacza to, że adresy przekierowań oraz endpointy pomocnicze nie są „zaszyte” w kodzie,
lecz mogą zostać zmienione bez ingerencji w logikę aplikacji.

Klasa jest jednocześnie komponentem Springa (\texttt{@Component}),
więc może być wstrzykiwana do klas konfiguracyjnych lub serwisów.
...
(\texttt{githubEmailEndpoint}), co jest użyteczne, gdy dostawca OAuth2 nie zwraca e-maila w profilu podstawowym.
...
% ------------------------------------------------------------
% JWT - komponenty: manager/generator/filter/service
% ------------------------------------------------------------
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/jwtManager}
%    \caption{Klasa \texttt{JwtManager}}
%    \label{fig:jwt-manager}
%\end{figure}
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/jwtAuthFilter}
%    \caption{Klasa \texttt{JwtAuthFilter}}
%    \label{fig:jwt-auth-filter}
%\end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/diagram-jwt-flow}
%     \caption{Uproszczony przepływ żądania dla endpointów prywatnych (JWT)}
%     \label{fig:diagram-jwt-flow}
% \end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/diagram-401-403}
%     \caption{Rozróżnienie odpowiedzi \texttt{401 Unauthorized} i \texttt{403 Forbidden} w aplikacji}
%     \label{fig:diagram-401-403}
% \end{figure}

% ------------------------------------------------------------
% Autoryzacja domenowa (Method Security)
% ------------------------------------------------------------
...
% ------------------------------------------------------------
% Miejsca na diagramy zintegrowane z opisem (do uzupełnienia)
% ------------------------------------------------------------

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/diagram-filter-chain-with-jwt}
%     \caption{Miejsce filtra JWT w łańcuchu filtrów Spring Security dla ścieżek prywatnych}
%     \label{fig:diagram-filter-chain-with-jwt}
% \end{figure}
