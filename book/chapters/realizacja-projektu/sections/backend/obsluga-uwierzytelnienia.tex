...
%diagram jak sie nasze wpisuja w to
...
\begin{itemize}
    \item \url{https://docs.spring.io/spring-security/reference/index.html}.
    \item \url{https://docs.spring.io/spring-security/reference/servlet/architecture.html}.
\end{itemize}
...
Twórcy frameworka skorzystali w tym celu ze wzorca \textit{Chain of Responsibility}
[TODO: odniesienie do opisu CoR]. % TODO: uzupelnic potem, jak wzorzec bedzie opisany
...
\newcommand{\innerheading}[1]{\par\medskip\noindent\textbf{#1}\par\smallskip}

\innerheading{Plik UrlsProperties}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/UrlsProperties/1}
    \caption{Klasa \texttt{UrlsProperties}}
    \label{fig:urls-properties}
\end{figure}

Klasa \texttt{UrlsProperties} pełni rolę centralnego magazynu adresów URL wykorzystywanych
w przepływach związanych z logowaniem i wylogowaniem użytkownika.
Została oznaczona adnotacją \texttt{@ConfigurationProperties(prefix = "url")},
dzięki czemu pola klasy mogą być w prosty sposób mapowane z pliku konfiguracyjnego
(np.\ \texttt{application.yml} / \texttt{application.properties}).
W praktyce oznacza to, że adresy przekierowań oraz endpointy pomocnicze nie są
„zaszyte” na stałe w kodzie, lecz mogą zostać zmienione bez ingerencji w logikę aplikacji.

Klasa jest jednocześnie komponentem Springa (\texttt{@Component}), więc może być wstrzykiwana
do klas konfiguracyjnych lub serwisów (np.\ w \texttt{SecurityConfig}).
Zawiera zestaw pól opisujących m.in.\ stronę po logowaniu, adres strony logowania OAuth2,
...
(\texttt{githubEmailEndpoint}), co jest przydatne w scenariuszu, w którym dostawca OAuth2
nie zwraca e-maila w podstawowym profilu użytkownika.

\innerheading{Plik JwtProperties}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/JwtProperties/1}
...
dzięki czemu wartości mogą pochodzić z konfiguracji środowiska (np.\ inne wartości w środowisku lokalnym i produkcyjnym).

Najważniejszym elementem klasy jest pole \texttt{key} typu \texttt{SecretKey}, które reprezentuje
klucz kryptograficzny używany do podpisywania i weryfikacji tokenów.
Pozostałe pola opisują zachowanie tokenu i sposób jego przechowywania po stronie klienta:
\texttt{tokenName} określa nazwę tokenu (np.\ nazwę cookie lub identyfikator używany w logice aplikacji),
...
Warto zauważyć, że klasa przyjmuje \texttt{SecretKey} w konstruktorze, co pozwala wstrzyknąć
klucz generowany lub dostarczany z zewnątrz, zamiast trzymać go jako stałą w kodzie.
Adnotacja \texttt{@Data} (Lombok) dostarcza gettery/settery i upraszcza zapis klasy.

\innerheading{Plik JwtConfig}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/JwtConfig/1}
...
W metodzie \texttt{jwtProperties()} tworzony jest nowy obiekt \texttt{JwtProperties},
a klucz \texttt{SecretKey} generowany jest przez bibliotekę \texttt{jjwt}
poprzez wywołanie \texttt{Jwts.SIG.HS256.key().build()}.

W efekcie aplikacja otrzymuje gotowy bean z kluczem do podpisywania tokenów HS256,
który może zostać użyty w komponentach odpowiedzialnych za generowanie i walidację JWT
(np.\ w klasach typu \texttt{JwtGenerator}, \texttt{JwtManager} czy filtrze \texttt{JwtAuthFilter}).

Z perspektywy architektury jest to świadome wydzielenie konfiguracji JWT do osobnej klasy:
logika generowania klucza jest skupiona w jednym miejscu, a reszta aplikacji korzysta już
z wstrzykniętego beana \texttt{JwtProperties}.

% TODO: za małe zdjecia
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-backendu/security/SecurityConfig/1}
    \caption{Konfiguracja security (1/4)}
...
\begin{figure}[H]
    ...
    \caption{Konfiguracja security (2/4)}
...
\begin{figure}[H]
    ...
    \caption{Konfiguracja security (3/4)}
...
\begin{figure}[H]
    ...
    \caption{Konfiguracja security (4/4)}
