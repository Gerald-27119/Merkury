...
\begin{itemize}
    ...
    w którym przeglądarka przesyła dane logowania zakodowane w nagłówku HTTP (\gls{naglowek-http}) \texttt{Authorization}.
...
Dla \glslink{endpoint}{endpointów} prywatnych zastosowano \gls{uwierzytelnianie} oparte o tokeny \glslink{jwt}{JWT}.
W tym podejściu serwer odtwarza tożsamość użytkownika dla każdego żądania
na podstawie \glslink{token}{tokena} przekazywanego przez klienta. Token jest przechowywany po stronie klienta
...
\begin{itemize}
    \item \textbf{\gls{http-401}} -- brak poprawnego uwierzytelnienia (brak tokenu lub błędny token).
    ...
    Ten przypadek jest obsługiwany przez \texttt{CustomAccessDeniedHandler}, który zwraca spójny format odpowiedzi \gls{json} z kodem 403.
\end{itemize}
