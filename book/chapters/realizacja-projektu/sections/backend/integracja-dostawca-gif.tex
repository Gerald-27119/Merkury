...
Jako źródło danych wykorzystano usługę \glslink{tenor}{Tenor}, a integrację rozdzielono na trzy warstwy:
\glslink{klient}{klienta} dostawcy, serwis aplikacyjny oraz kontroler \glslink{rest_api}{REST}.
...
\begin{figure}[H]
    ...
    \caption{\texttt{GifController}}
    \label{fig:gif:controller}
\end{figure}

Rysunek~\ref{fig:gif:controller} przedstawia kontroler \glslink{rest_api}{REST} udostępniający funkcjonalności \glslink{gif}{GIF-ów}
na potrzeby \glslink{frontend}{frontendu}. Udostępniono dwa \glslink{endpoint}{endpointy}:
\begin{itemize}
    \item \texttt{GET /gifs/trending} -- zwraca listę popularnych kategorii,
    \item \texttt{GET /gifs/search} -- zwraca wyniki wyszukiwania dla podanej frazy, ze wsparciem \glslink{paginacja}{paginacji}.
\end{itemize}
