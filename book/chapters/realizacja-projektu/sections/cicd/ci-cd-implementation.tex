...
Jest on uruchamiany:
\begin{itemize}
    \item przy zdarzeniu \texttt{push} dla zmian w katalogu \texttt{vulcanus/**},
    \item przy otwarciu \textit{pull request} do gałęzi \texttt{master} oraz \texttt{develop},
    \item w trybie \texttt{merge\_group} (kolejka scalania) dla gałęzi \texttt{master} oraz \texttt{develop},
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Fragment pliku \texttt{java-ci.yml}: konfiguracja wyzwalaczy \glslink{workflow}{workflow} (\texttt{on:}).}
...
\begin{figure}[H]
    ...
    \caption{Podsumowanie wykonania \glslink{workflow}{workflow} \glslink{backend}{backendu} (\texttt{java-ci.yml}) w GitHub Actions.}
...
\begin{figure}[H]
    ...
    \caption{Fragment pliku \texttt{java-ci.yml}: definicja \glslink{job}{joba} \texttt{setup}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-cicd/github-backend-job1}
    \caption{Przykładowy log wykonania \glslink{job}{joba} \texttt{setup} w \glslink{github-actions}{GitHub Actions}.}
...
\begin{itemize}
    ...
    \item przywracany jest cache repozytorium Mavena (przyspieszenie pobierania zależności),
...
\begin{figure}[H]
    ...
    \caption{Fragment pliku \texttt{java-ci.yml}: definicja \glslink{job}{joba} \texttt{build}.}
...
\begin{figure}[H]
    ...
    \caption{Przykładowy log wykonania \glslink{job}{joba} \texttt{build} w \glslink{github-actions}{GitHub Actions}.}
...
W jego ramach przywracany jest cache Mavena, a następnie uruchamiane są testy \glslink{backend}{backendu}.
...
\begin{figure}[H]
    ...
    \caption{Fragment pliku \texttt{java-ci.yml}: definicja \glslink{job}{joba} \texttt{test} (wraz z użyciem \texttt{secrets}).}
...
\begin{figure}[H]
    ...
    \caption{Przykładowy log wykonania \glslink{job}{joba} \texttt{test} w \glslink{github-actions}{GitHub Actions}s.}
...
dla otwieranych \textit{pull request} do gałęzi \texttt{master} i \texttt{develop} oraz w trybie
\texttt{merge\_group}, z pominięciem zmian w \texttt{book/**}.
...
\begin{figure}[H]
    ...
    \caption{Fragment pliku \texttt{react-ci.yml}: konfiguracja \glslink{workflow}{workflow} oraz
    \glslink{job}{joba} \texttt{formatting-and-test}.}
...
Najpierw pobierany jest kod i ustawiana jest odpowiednia wersja Node.js, następnie instalowane są zależności.
...
\begin{figure}[H]
    ...
    \caption{Podsumowanie wykonania \glslink{workflow}{workflow} \glslink{frontend}{frontendu}
    (\texttt{react-ci.yml}) w \glslink{github-actions}{GitHub Actions}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-cicd/github-frontend-job}
    \caption{Przykładowy log wykonania \glslink{job}{joba} \texttt{formatting-and-test} w \glslink{github-actions}{GitHub Actions}.}
