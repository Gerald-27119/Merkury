...
W niniejszym rozdziale przedstawiono implementację panelu użytkownika po stronie frontendowej.
...
\begin{itemize}
    ...
    \item \textbf{Spots} -- listy spotów dodanych do różnych kategorii (np. ulubione, do ponownego odwiedzenia);
    \item \textbf{Photos} -- lista zdjęć dodanych przez użytkownika do spotów, komentarzy pod spotami oraz na forum;
    \item \textbf{Movies} -- lista filmów dodanych przez użytkownika do spotów, komentarzy pod spotami oraz na forum;
    \item \textbf{Social} -- listy znajomych, obserwowanych i obserwujących;
    \item \textbf{Add spot} -- lista spotów dodanych przez użytkownika oraz formularz dodawania nowego spota;
...
Dane profilu są pobierane za pomocą hooka \texttt{useQuery} z biblioteki TanStack Query, który odwołuje
się do odpowiedniego endpointu backendu.
...
Nazwa użytkownika pobierana jest ze ścieżki URL za pomocą hooka \texttt{useParams}, a dane profilu -- za
pomocą \texttt{useQuery}, które wysyła zapytanie do backendu z przekazaną nazwą.
...
\begin{itemize}
    ...
    akcje \textit{follow} oraz zarządzanie znajomymi;
    \item komponent \texttt{Modal} do potwierdzania usunięcia znajomego lub zaprzestania obserwowania.
\end{itemize}
...
Relacje społecznościowe modyfikowane są przy użyciu kilku mutacji po stronie API, odpowiadających za dodawanie
lub usuwanie znajomych, zarządzanie listą obserwowanych oraz akceptowanie lub odrzucanie zaproszeń.
Po powodzeniu mutacji odświeżane są dane profilu użytkownika.
Jeżeli z backendu zwrócona zostanie informacja, że jest to profil zalogowanego użytkownika, w hooku \texttt{useEffect}
następuje przekierowanie na adres \texttt{/account/profile}.
...
Dla własnego profilu wykorzystano hook \texttt{useMutation}, który umożliwia zmianę zdjęcia.
Po najechaniu kursorem na zdjęcie pojawia się półprzezroczysty overlay z ikoną edycji i napisem
\texttt{"Change profile photo."}, a kliknięcie uruchamia wybór nowego pliku.
Po udanej aktualizacji zdjęcia odświeżane są dane profilu oraz wyświetlany jest komunikat sukcesu; w przypadku
błędu prezentowany jest odpowiedni komunikat informujący o niepowodzeniu aktualizacji.
...
Komponent prezentuje pojedynczą statystykę profilu (np. liczbę znajomych) w formie kafelka,
...
Zakładka \texttt{Spots} odpowiada za prezentację list spotów przypisanych do różnych kategorii
...
(np. wszystkie, ulubione, planowane do odwiedzenia),
a aktualnie wybrany typ przechowywany jest w stanie za pomocą hooka \texttt{useState}.

Dane spotów są pobierane za pomocą hooka \texttt{useInfiniteQuery}, który wysyła zapytania do
backendu z uwzględnieniem wybranego typu listy.
Mechanizm \textit{infinite scrolla} realizowany jest przy użyciu \texttt{IntersectionObserver},
...
Podczas ładowania danych wyświetlany jest \texttt{LoadingSpinner}.
Jeżeli użytkownik nie posiada żadnych spotów w wybranej liście, prezentowany jest komunikat:
...
Komponent reprezentuje pojedynczy kafelek spota.
...
otwarcia spota na mapie i usunięcia z listy.

Do modyfikacji listy wykorzystywana jest mutacja obsługująca dodawanie i usuwanie spotów z danej kategorii.
Po powodzeniu odświeżane są dane listy, tak aby kafelek został usunięty z widoku.
Przed usunięciem wyświetlany jest modal potwierdzenia (komponent \texttt{Modal}), którego stan widoczności
kontrolowany jest za pomocą hooka \texttt{useBoolean}.

\textbf{\texttt{FavoriteSpotTags}}
Komponent prezentuje listę tagów spota jako niewielkie etykiety z nazwą tagu.
...
Do zarządzania sortowaniem i filtrowaniem po dacie wykorzystywany jest customowy hook \texttt{useDateSortFilter},
który zwraca typ sortowania oraz zakres dat.
Dane są pobierane za pomocą \texttt{useInfiniteQuery}, które wysyła zapytania do backendu z uwzględnieniem aktualnych filtrów.
Mechanizm \textit{infinite scroll} zrealizowano przy użyciu \texttt{IntersectionObserver}.

W przypadku błędu podczas pobierania zdjęć w hooku \texttt{useEffect} ustawiany jest komunikat błędu w komponencie \texttt{Notification}.
...
Ponownie wykorzystywany jest hook \texttt{useDateSortFilter}, a dane pobierane są za pomocą \texttt{useInfiniteQuery}
...
Do odtwarzania wykorzystano bibliotekę \texttt{react-player} oraz zestaw komponentów kontrolnych z biblioteki \texttt{media-chrome}.
Stan odtwarzania i ewentualnego błędu zarządzany jest za pomocą customowego hooka \texttt{useBoolean}.
...
\newline Aktualnie wybrany typ listy (\texttt{FRIENDS}, \texttt{FOLLOWED}, \texttt{FOLLOWERS}) jest pobierany ze store Reduxa
za pomocą \texttt{useSelectorTyped}.
...
z parametrem \texttt{isSocialForViewer = false}.
...
tego użytkownika -- z odpowiednich endpointów powiązanych w rozszerzonym \texttt{queryConfigMap}.

W zależności od typu listy pobierane są dane użytkowników (\texttt{SocialDto}) lub zdjęć (\texttt{DatedMediaGroup}).
Po spłaszczeniu danych całość przekazywana jest do komponentu \texttt{Social} z parametrem \texttt{isSocialForViewer = true}.

\textbf{\texttt{Social}}
Jest to główny, uniwersalny komponent wykorzystywany zarówno dla widoku własnego, jak i cudzego profilu.
Widok opakowany jest w \texttt{AccountWrapper} z wariantem \texttt{SOCIAL}, a nagłówek sekcji wyświetlany
przez \texttt{AccountTitle} z tekstem \texttt{"social list"}.
...
Zmiana typu listy zapisywana jest w store Reduxa za pomocą akcji \texttt{socialAction.setType}.
...
\end{itemize}
Na dole listy znajduje się element powiązany z \texttt{loadMoreRef}, obsługujący \textit{infinite scroll}, oraz
...
Wyświetlane są: zdjęcie profilowe, nazwa użytkownika oraz zestaw przycisków akcji, m.in. przejście do profilu,
otwarcie prywatnego czatu oraz dodanie lub usunięcie z listy znajomych bądź obserwowanych.

Relacje znajomości i obserwowania modyfikowane są za pomocą mutacji po stronie API, odpowiedzialnych za zarządzanie
listą znajomych oraz obserwowanych.
Po ich powodzeniu odświeżane są odpowiednie listy.
Otwarcie lub utworzenie prywatnego czatu realizowane jest przez osobną mutację, która zwraca obiekt czatu; następnie
jest on zapisywany w store Reduxa, ustawiany jako aktualnie wybrany, a aplikacja przełącza widok na \texttt{/chat}.

Przed usunięciem znajomego lub zakończeniem obserwowania wyświetlany jest modal potwierdzenia, którego stan zarządzany
jest przez hook \texttt{useBoolean}.
...
Umożliwia m.in. wyróżnienie przycisku aktywnego oraz dostosowanie szerokości do treści.
...
Zapytanie tekstowe jest debouncowane za pomocą hooka \texttt{useDebounce}, a dane pobierane są z użyciem
\texttt{useInfiniteQuery}, które wysyła zapytania wyszukujące użytkowników po nazwie.
...
Zmiana statusu zaproszenia (zaakceptowanie lub odrzucenie) realizowana jest przez mutację, która wysyła odpowiednie żądanie do backendu.
...
Zakładka \texttt{Add spot} służy do zarządzania spotami dodanymi przez użytkownika oraz do dodawania nowych spotów.
...
Dodatkowo wykorzystywany jest schemat walidacji \texttt{spotSchema} utworzony z użyciem biblioteki \texttt{zod}.

\textbf{\texttt{AddedSpot}}
Jest to główny komponent zakładki.
Odpowiada za wyświetlenie listy spotów dodanych przez użytkownika oraz za otwieranie okna modalnego
\texttt{AddSpotModal} służącego do dodania nowego spota.
Dane pobierane są za pomocą \texttt{useInfiniteQuery}, które pobiera kolejne strony wyników z backendu,
a mechanizm \textit{infinite scroll} zaimplementowano przy użyciu \texttt{IntersectionObserver}.

Stan widoczności modala zarządzany jest przez hook \texttt{useBoolean}.
Dodatkowo sprawdzana jest szerokość okna przeglądarki; formularz dodawania spota dostępny jest tylko
...
Jeżeli użytkownik nie dodał żadnych spotów, wyświetlany jest komunikat:
...
Komponent służy do wyboru i podglądu multimediów (zdjęć i filmów) dodawanych do spota.
...
Wykorzystywana jest biblioteka \texttt{maplibregl} oraz przygotowany styl mapy \texttt{/map\_style1.json}.
Na mapie umieszczany jest marker w pozycji spota.

\textbf{\texttt{PolygonDrawer}}
Komponent umożliwia interaktywne narysowanie konturu spota na mapie.
...
Komponent odpowiada za dodawanie nowego spota.
W stanie przechowywany jest obiekt z danymi spota, komunikaty błędów walidacyjnych oraz aktualna pozycja mapy.
Adres budowany jest z pól formularza (ulica, miasto, region, kraj) i debouncowany za pomocą hooka \texttt{useDebounce}.
Na podstawie pełnego adresu hook \texttt{useQuery} pobiera współrzędne geograficzne, a mapa przesuwana
...
Po poprawnej walidacji wywoływana jest mutacja odpowiedzialna za dodanie spota po stronie backendu.
Po jej powodzeniu modal jest zamykany, formularz resetowany, dane listy odświeżane, a użytkownik otrzymuje
komunikat o poprawnym dodaniu spota.

Wyświetlenie modala zrealizowano za pomocą \texttt{createPortal}, co umożliwia umieszczenie go w dedykowanym elemencie
DOM (\texttt{<div id="modal">}).
Za animacje otwierania i zamykania odpowiadają komponenty \texttt{AnimatePresence} oraz
\texttt{motion.div} z biblioteki motion.

\textbf{\texttt{AddSpotInput}}
Komponent odpowiada za wyświetlanie i obsługę listy pól formularza.
Korzysta z lokalnego stanu błędów oraz schematu \texttt{spotSchema} (za pomocą \texttt{pick}) do walidacji pojedynczych pól.
Do prezentacji pól wykorzystuje uniwersalny komponent \texttt{FormInput}.

\textbf{\texttt{AddedSpotTile}}
Komponent wyświetla pojedynczy kafelek z informacjami o dodanym spocie.
...
Zakładka \texttt{Comments} wyświetla wszystkie komentarze dodane przez użytkownika pod spotami.
...
Komponent korzysta z hooka \texttt{useDateSortFilter}, analogicznie jak zakładki \texttt{Photos} i \texttt{Movies}.
Dane pobierane są za pomocą \texttt{useInfiniteQuery}, które pobiera kolejne strony komentarzy z backendu,
a \textit{infinite scroll} realizowany jest przez \texttt{IntersectionObserver}.

W przypadku błędu podczas pobierania komentarzy w hooku \texttt{useEffect} ustawiany jest komunikat błędu,
...
Poniżej znajduje się panel filtrów \texttt{SortAndDateFilters}, a następnie lista komentarzy pogrupowanych po dacie i nazwie spota.

Dla każdej grupy wyświetlany jest \texttt{DateBadge} z datą oraz informacją o spocie, a poniżej lista komentarzy
renderowana przez \texttt{CommentsList}.
Jeżeli użytkownik nie dodał żadnych komentarzy, wyświetlany jest komunikat:
\texttt{"You haven't added any comments."}.
Podczas ładowania danych oraz kolejnych stron prezentowany jest \texttt{LoadingSpinner}.
...
Na początku, za pomocą hooka \texttt{useQuery}, pobierane są dane użytkownika (nazwa, e-mail, dostawca logowania).
Dopóki dane są ładowane, wyświetlany jest \texttt{LoadingSpinner}.
...
Walidacja realizowana jest przy użyciu schematów \texttt{baseSchemas} zdefiniowanych w bibliotece \texttt{zod}, a
obsługę formularza zapewnia \texttt{react-hook-form} w połączeniu z \texttt{zodResolver}.
Dane wysyłane są do backendu za pomocą mutacji, która aktualizuje odpowiednie ustawienia użytkownika.
W przypadku błędu prezentowany jest komunikat z odpowiedzi backendu; po sukcesie wyświetlany jest komunikat o
poprawnej zmianie ustawień oraz odświeżane są dane użytkownika.
...
Jeżeli konto zostało utworzone za pomocą zewnętrznego dostawcy (np. Google, GitHub), wyświetlany jest komunikat:
...
Komponent prezentuje pojedyncze pole z aktualną wartością danej informacji (np. nazwy użytkownika) w trybie
...
Przyjmuje wariant widoku i na jego podstawie stosuje odpowiednie klasy Tailwind, dzięki czemu zachowana jest
spójność wizualna przy jednoczesnej możliwości dostosowania przestrzeni danej sekcji.
...
Stan rozwinięcia listy zarządzany jest przez hook \texttt{useBoolean}, a animacje zrealizowano za pomocą
biblioteki \texttt{framer-motion}.
Komponent używany jest pośrednio poprzez \texttt{SortAndDateFilters}.

\textbf{\texttt{DateChooser}}
Jest to wrapper wokół komponentu \texttt{DatePicker} z biblioteki Ant Design.
Służy do wyboru daty w polach \texttt{"From"} oraz \texttt{"To"} w panelu filtrowania.

\textbf{\texttt{SortAndDateFilters}}
Komponent łączy \texttt{SortDropdown} oraz dwa \texttt{DateChooser}, tworząc panel sterujący sortowaniem i
...
\textbf{\texttt{LoadingSpinner}}jest wspólnym komponentem informującym o
trwającym ładowaniu danych i pojawia się
w większości widoków w stanach \texttt{isLoading} oraz \texttt{isFetchingNextPage}.

\textbf{\texttt{Notification}} (przywoływany pośrednio poprzez akcje Reduxa \newline
