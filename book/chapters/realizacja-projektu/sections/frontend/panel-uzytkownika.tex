...
W niniejszym rozdziale przedstawiono implementację panelu użytkownika po stronie \glslink{frontend}{frontendowej}.
...
\begin{itemize}
    ...
    \item \textbf{Spots} -- listy \glslink{spot}{spotów} dodanych do różnych kategorii (np. ulubione, do ponownego odwiedzenia);
...
Relacje społecznościowe modyfikowane są przy użyciu kilku mutacji po stronie \glslink{api}{API}, odpowiadających za dodawanie
...
Po najechaniu kursorem na zdjęcie pojawia się półprzezroczyste tło z ikoną edycji i napisem
...
\begin{figure}[H]
    ...
    \caption{Komponent \texttt{Social} (1).}
...
\begin{figure}[H]
    ...
    \caption{Komponent \texttt{Social} (2).}
...
\textbf{\texttt{Social}}  (rys. \ref{img:social})
...
Zmiana typu listy zapisywana jest w store \glslink{redux}{Reduxa} za pomocą akcji \texttt{socialAction.setType}.
...
Relacje znajomości i obserwowania modyfikowane są za pomocą mutacji po stronie \glslink{api}{API}, odpowiedzialnych za zarządzanie
...
Wykorzystywana jest \glslink{biblioteka}{biblioteka} \texttt{maplibregl} oraz przygotowany styl mapy \texttt{/map\_style1.json}.
...
Adres budowany jest z pól formularza (ulica, miasto, region, kraj) i debouncowany za pomocą \glslink{hook}{hooka} \texttt{useDebounce}.
...
Po poprawnej walidacji wywoływana jest mutacja odpowiedzialna za dodanie \glslink{spot}{spota} po stronie \glslink{backend}{backendu}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/date-chooser}
