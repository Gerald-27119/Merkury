...
\begin{itemize}
    \item \textbf{Profil użytkownika} -- wraz z możliwością zmiany zdjęcia profilowego;
    \item \textbf{Listy spotów} -- dodanych do różnych kategorii (ulubione, planowane do odwiedzenia,
    odwiedzone i ocenione pozytywnie, odwiedzone i ocenione negatywnie);
    \item \textbf{Lista zdjęć} -- dodanych przez użytkownika do \glslink{spot}{spotów}, komentarzy pod \glslink{spot}{spotami} oraz na forum;
    \item \textbf{Lista filmów} -- dodanych przez użytkownika do \glslink{spot}{spotów}, komentarzy pod \glslink{spot}{spotami} oraz na forum;
    \item \textbf{Społeczność} -- listy znajomych, obserwowanych i obserwujących;
    \item \textbf{Dodanie spota} -- lista \glslink{spot}{spotów} dodanych przez użytkownika oraz formularz dodawania nowego \glslink{spot}{spota};
    \item \textbf{Komentarze} -- lista komentarzy dodanych przez użytkownika pod \glslink{spot}{spotami};
    \item \textbf{Ustawienia} -- zmiana nazwy użytkownika, adresu e-mail oraz hasła.
\end{itemize}
...
Zakładka odpowiada za prezentację profilu użytkownika.
...
\textbf{\texttt{UserOwnProfile}} (rys. \ref{img:user-own-profile})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/user-own-profile}
    \caption{Komponent \texttt{UserOwnProfile}.}
    \label{img:user-own-profile}
\end{figure}

\textbf{\texttt{ProfileForViewer}} (rys. \ref{img:profile-for-viewer})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/profile-for-viewer}
    \caption{Komponent \texttt{ProfileForViewer}.}
    \label{img:profile-for-viewer}
\end{figure}
...
%-----------------------------------
\subsubsection{Listy spotów}
\label{subsubsec:spots}

Zakładka odpowiada za prezentację list \glslink{spot}{spotów} przypisanych do różnych kategorii
...
\textbf{\texttt{FavoriteSpots}}  (rys. \ref{img:favorite-spots})
...
a aktualnie wybrany typ przechowywany jest w \glslink{stan}{stanie} za pomocą \glslink{hook}{hooka} \texttt{useState}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/favorite-spots}
    \caption{Komponent \texttt{FavoriteSpots}.}
    \label{img:favorite-spots}
\end{figure}
...
Przed usunięciem wyświetlany jest modal potwierdzenia (komponent \glslink{modal}{\texttt{Modal}}), którego
\glslink{stan}{stan} widoczności kontrolowany jest za pomocą \glslink{hook}{hooka} \texttt{useBoolean}.

\textbf{\texttt{FavoriteSpotTags}}
Komponent prezentuje listę tagów \glslink{spot}{spota} jako niewielkie etykiety z nazwą tagu.

%-----------------------------------
\subsubsection{Lista zdjęć}
\label{subsubsec:photos}

Zakładka odpowiada za prezentację zdjęć dodanych przez użytkownika.
...
\textbf{\texttt{Photos}}  (rys. \ref{img:photos})
...
Mechanizm \glslink{infinite-scroll}{\textit{infinite scroll}} zrealizowano przy użyciu \newline \texttt{IntersectionObserver}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/photos}
    \caption{Komponent \texttt{Photos}.}
    \label{img:photos}
\end{figure}
...
\textbf{\texttt{Media}}  (rys. \ref{img:media})
Jest to uniwersalny komponent używany zarówno w zakładce \texttt{Lista zdjęć}, jak i \texttt{Lista filmów} (por. sekcja~\ref{subsubsec:movies}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/media}
    \caption{Komponent \texttt{Media}.}
    \label{img:media}
\end{figure}

%-----------------------------------
\subsubsection{Lista filmów}
\label{subsubsec:movies}

Zakładka odpowiada za prezentację filmów dodanych przez użytkownika.
Pod względem struktury jest bardzo podobna do zakładki \texttt{Lista zdjęć} (por. sekcja~\ref{subsubsec:photos}) i
...
\textbf{\texttt{Movies}}  (rys. \ref{img:movies})
Komponent konfiguruje widok filmów i przekazuje dane do \texttt{Media} z wariantem \texttt{AccountWrapperType.MOVIES}.
Ponownie wykorzystywany jest \glslink{hook}{hook} \newline \texttt{useDateSortFilter}, a dane pobierane są za pomocą \texttt{useInfiniteQuery}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/movies}
    \caption{Komponent \texttt{Movies}.}
    \label{img:movies}
\end{figure}

\textbf{\texttt{Movie}}
Komponent reprezentuje pojedynczy film.
Do odtwarzania wykorzystano \glslink{biblioteka}{bibliotekę} \texttt{react-player} oraz zestaw komponentów kontrolnych z
\glslink{biblioteka}{biblioteki} \newline \texttt{media-chrome}.
...
Zakładka odpowiada za obsługę funkcji społecznościowych: znajomych, obserwowanych, obserwujących
...
\textbf{\texttt{UserOwnSocial}}  (rys. \ref{img:user-own-social1} i \ref{img:user-own-social2})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/user-own-social1}
    \caption{Komponent \texttt{Social} (1/2).}
    \label{img:user-own-social1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/user-own-social2}
    \caption{Komponent \texttt{Social} (2/2).}
    \label{img:user-own-social2}
\end{figure}
...
W zależności od typu listy pobierane są dane użytkowników (\texttt{SocialDto}) lub zdjęć \newline (\texttt{DatedMediaGroup}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/social}
    \caption{Komponent \texttt{Social}.}
    \label{img:social}
\end{figure}
...
Przed usunięciem znajomego lub zakończeniem obserwowania wyświetlany jest modal potwierdzenia, którego \glslink{stan}{stan} zarządzany
...
%-----------------------------------
\subsubsection{Dodanie spota}
\label{subsubsec:add-spot}

Zakładka służy do zarządzania \glslink{spot}{spotami} dodanymi przez użytkownika oraz do
...
\textbf{\texttt{AddedSpot}}  (rys. \ref{img:added-spot})
Jest to główny komponent zakładki.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/added-spot}
    \caption{Komponent \texttt{AddedSpot}.}
    \label{img:added-spot}
\end{figure}

\textbf{\texttt{UploadButton}}
Komponent służy do wyboru i podglądu multimediów (zdjęć i filmów) dodawanych do \glslink{spot}{spota}.
W lokalnym \glslink{stan}{stanie} utrzymywana jest lista wybranych plików wraz z tymczasowymi adresami tworzonymi za pomocą
...
W \glslink{stan}{stanie} przechowywany jest obiekt z danymi \glslink{spot}{spota}, komunikaty błędów walidacyjnych oraz aktualna pozycja mapy.
...
Prezentowane są: zdjęcie \glslink{spot}{spota}, mini mapa (\texttt{SpotMap}) z lokalizacją, nazwa, opis oraz adres (kraj, miasto, ulica).

%-----------------------------------
\subsubsection{Komentarze}
\label{subsubsec:comments}

Zakładka wyświetla wszystkie komentarze dodane przez użytkownika pod \glslink{spot}{spotami}.
...
\textbf{\texttt{Comments}}  (rys. \ref{img:comments})
Komponent korzysta z \glslink{hook}{hooka} \texttt{useDateSortFilter}, analogicznie jak zakładki \texttt{Lista zdjęć} i \texttt{Lista filmów}.
Dane pobierane są za pomocą \texttt{useInfiniteQuery}, które pobiera kolejne strony komentarzy z \glslink{backend}{backendu},
a \glslink{infinite-scroll}{\textit{infinite scroll}} realizowany jest przez \newline \texttt{IntersectionObserver}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/comments}
    \caption{Komponent \texttt{Comments}.}
    \label{img:comments}
\end{figure}
...
Zakładka umożliwia użytkownikowi edycję podstawowych danych konta.
...
\textbf{\texttt{Settings}}  (rys. \ref{img:settings})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/settings}
    \caption{Komponent \texttt{Settings}.}
    \label{img:settings}
\end{figure}
...
\textbf{\texttt{AccountWrapper}} (rys. \ref{img:account-wrapper})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/account-wrapper}
    \caption{Komponent \texttt{AccountWrapper}.}
    \label{img:account-wrapper}
\end{figure}

\textbf{\texttt{AccountTitle}} (rys. \ref{img:account-title})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/account-title}
    \caption{Komponent \texttt{AccountTitle}.}
    \label{img:account-title}
\end{figure}
...
Komponent używany jest pośrednio poprzez \newline \texttt{SortAndDateFilters}.

\textbf{\texttt{DateChooser}} (rys. \ref{img:date-chooser})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/date-chooser}
    \caption{Wybór zakresu dat w panelu filtrowania (\texttt{DateChooser}).}
    \label{img:date-chooser}
\end{figure}

\textbf{\texttt{SortAndDateFilters}} (rys. \ref{img:sort-and-date-filters})
...
Wykorzystywany jest w zakładkach \texttt{Lista zdjęć}, \texttt{Lista filmów} oraz \texttt{Komentarze}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/sort-and-date-filters}
    \caption{Komponent \texttt{SortAndDateFilters}.}
    \label{img:sort-and-date-filters}
\end{figure}

\textbf{\texttt{DateBadge}} (rys. \ref{img:date-badge})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/panel-uzytkownika/date-badge}
    \caption{Komponent \texttt{DateBadge}.}
    \label{img:date-badge}
\end{figure}
