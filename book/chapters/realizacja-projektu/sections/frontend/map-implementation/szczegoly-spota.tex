...
W lokalnym stanie (\texttt{useState}) ustawiany jest obecny indeks elementu, a poprzez porównanie go z
...
przyciski do odtwarzania i pauzowania) zbudowany został z elementów \glslink{biblioteka}{biblioteki}
\texttt{media-chrome}.
...
Przyciski wyświetlane są poprzez \newline \texttt{SpotActionButton}, którym poszczególne funkcje przekazywane są
...
skopiowanie odpowiedniego linka do pamięci podręcznej systemu korzystając z \newline \texttt{navigator.clipboard}.
...
\glslink{react-component}{komponencie} \newline \texttt{Notification} wyświetla odpowiedni komunikat.
Gdy ten warunek jest spełniony, otwierany jest \glslink{modal}{modal} \newline \texttt{SpotAddMediaModal}, zwierający
...
znajduje się w liście polubionych użytkownika.
...
szczegółów \glslink{spot}{spota} jest unieważnianie, dzięki czemu dodane media będą znajdywać się w
...
\glslink{react-component}{Komponent} wyświetlający listę komentarzy przewijanej przy użyciu
\glslink{infinite-scroll}{infinite scroll}.
Mechanizm ten jest realizowany za pomocą \newline \texttt{IntersectionObserver}, który nasłuchuje na element powiązany
...
Inną wymaganą pozycją jest pole tekstowe, w którym użytkownik umieszcza swoją opinię.
...
\newline \texttt{SpotCommentAuthor} zawiera informacje o autorze komentarza: nazwę oraz zdjęcie profilowe,
