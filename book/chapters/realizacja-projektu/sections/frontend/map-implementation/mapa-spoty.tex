...
lokalizację, na którą wskazuje mapa oraz poziom przybliżenia widoku.
...
\glslink{spot}{spota}.
Jeśli tak jest, odczytywane są odpowiednie dane, a obraz mapy przekierowany jest na wskazaną lokalizację oraz
...
Pozwala to dodać do wyświetlanych elementów event-listener'y obsługujące najechanie myszką na dany element oraz funkcji
...
\glslink{react-component}{Komponent} obsługujący ustawianie przybliżenia mapy.
...
\newline \texttt{useInfiniteQuery} z \glslink{biblioteka}{biblioteki} \texttt{Tanstack Query}.
...
Filtrowanie \glslink{spot}{spotów} po nazwie obsługuje \newline \texttt{CurrentViewSpotsNameSearchBar}.
...
Za pomocą \texttt{Rate} z \glslink{biblioteka}{biblioteki} \texttt{antd} wybierana jest wartość oceny, a następnie
...
Wybrana opcja sortowania przechowywana jest w lokalnym stanie (\texttt{useState}), \glslink{hook}{hook} \texttt{useEffect}
