...
także ikony obrazującej obecny stan warunków atmosferycznych (\glslink{react-component}{komponent} \texttt{WeatherIcon (por. sekcja~\ref{subsubsec:komponent-wspolny})}).
...
(\glslink{react-component}{komponent} \texttt{DetailedSpotWeather})
...
Został zbudowany z dziesięciu \glslink{react-component}{komponentów}:
\begin{itemize}
...
\textbf{\texttt{DetailedSpotWeather}} (rys. \ref{fig:detailed-spot-weather-0} i \ref{fig:detailed-spot-weather-2})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/mapa/pogoda/detailed_spot_weather0}
    \caption{Implementacja komponentu DetailedSpotWeather (1/3)}
    \label{fig:detailed-spot-weather-0}
\end{figure}
\noindent
...
\begin{figure}[H]
    ...
    \caption{Implementacja komponentu DetailedSpotWeather (2/3)}
...
\begin{figure}[H]
    ...
    \caption{Implementacja komponentu DetailedSpotWeather (3/3)}
    \label{fig:detailed-spot-weather-2}
\end{figure}
\noindent


\textbf{\texttt{WeatherOverview}} (rys. \ref{fig:weather-overview-1} i \ref{fig:weather-overview-3})
...
się co minutę.
Po usunięciu \glslink{react-component}{komponentu} z drzewa renderowania, funkcja czyszcząca usunie interwał.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/mapa/pogoda/weather-overview-1}
    \caption{Implementacja komponentu WeatherOverview (1/3)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/mapa/pogoda/weather-overview-2}
    \caption{Implementacja komponentu WeatherOverview (2/3)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/mapa/pogoda/weather-overview-3}
    \caption{Implementacja komponentu WeatherOverview (3/3)}
    \label{fig:weather-overview-3}
\end{figure}
\noindent
...
Indeks UV przedstawiony jest słownie poprzez funkcję \newline \texttt{getUvIndexTextLevel}, która określa go na podstawie
...
W tym \glslink{react-component}{komponencie} wyświetlane są prędkości wiatru na różnych wysokościach w \texttt{km\textbackslash h} lub \texttt{m\textbackslash s}.
...
W tym rozdziale przedstawiono \glslink{react-component}{komponent}, z którego korzystają obydwie części modułu pogody.
...
\glslink{react-component}{Komponent} opcjonalnie przyjmuje parametr określający rozmiar ikony (\emph{textSize}), będący klasą \glslink{tailwind-css}{Tailwind}.
Jeśli nie zostanie podany, ustawiona zostanie domyślna opcja \emph{text-3xl}.
Poniżej przedstawiono implementację tego \glslink{react-component}{komponentu}.
