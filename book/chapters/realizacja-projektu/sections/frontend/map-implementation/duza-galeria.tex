...
Jego otwarcie oraz zamknięcie animowane jest przy użyciu \glslink{biblioteka}{biblioteki} \texttt{motion}.

\textbf{\texttt{ExpandedGallerySidebar}}
Wyświetla listę zdjęć lub filmów przewijaną przy użyciu \texttt{\glslink{infinite-scroll}{infinite scroll}}.
Dane pobierane są \glslink{paginacja}{paginacyjnie} z \glslink{backend}{backendu} przy pomocy \glslink{hook}{hook'a}
...
Pierwszy element z listy jest wyświetlany w \glslink{react-component}{komponencie} \newline \texttt{ExpandedMediaDisplay}.
...
Obsługa video odbywa się za pomocą elementów \glslink{biblioteka}{biblioteki} \texttt{react-player}.
W trakcie pobierania danych wyświetlany jest \texttt{LoadingSpinner}, a o błędzie informuje komunikat
...
media.
Polubienie elementu realizowane jest poprzez funkcję \newline \texttt{handleClickEditSpotMediaLikes}.
...
W celu utrzymania aktualności danych, powodzenie operacji spowoduje unieważnienie wszystkich zapytań
