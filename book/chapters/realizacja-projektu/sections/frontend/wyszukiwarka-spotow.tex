...
Jednym z głównych modułów aplikacji jest wyszukiwarka spotów, umożliwiająca szybkie
...
W trybie prostym prezentowana jest karuzela z dwunastoma najpopularniejszymi \glslink{spot}{spotami} w całej aplikacji.
W tym widoku możliwe jest wyszukiwanie \glslink{spot}{spotów} po lokalizacji (kraj, region, miasto).
...
Wyszukiwarka spotów została zbudowana z dwóch głównych komponentów: \newline \texttt{HomePage} oraz \texttt{AdvanceHomePage}.
...
Komponent \texttt{Switch} zawiera dwa elementy \texttt{NavLink} z \glslink{biblioteka}{biblioteki} React Router,
...
Po pojawieniu się listy możliwe jest wybranie odpowiedniej lokalizacji,
co ułatwia określenie lokalizacji dostępnych \glslink{spot}{spotów}.
...
\begin{itemize}
    ...
    \item dwa przyciski: jeden prowadzący do widoku szczegółów \glslink{spot}{spota} oraz drugi informujący,
        jak daleko znajduje się dany \glslink{spot}{spot}; po kliknięciu przycisku prezentowana jest lokalizacja \glslink{spot}{spota} na mapie.
\end{itemize}
