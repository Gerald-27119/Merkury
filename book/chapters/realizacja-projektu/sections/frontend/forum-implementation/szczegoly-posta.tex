...
\glslink{react-component}{Komponent} \texttt{ForumThread} (rys. \ref{img:forum_thread-1}~\ref{img:forum_thread-2}) odpowiada za wyświetlanie pełnej treści pojedynczego posta wraz z komentarzami.
...
\begin{figure}[H]
    ...
    \caption{Komponent ForumThread (1/2)}
...
\begin{figure}[H]
    ...
    \caption{Komponent ForumThread (2/2)}
    \label{img:forum_thread-2}
\end{figure}

\texttt{DetailedPost} (rys. \ref{img:detailed-post}) składa się z następujących \glslink{react-component}{komponentów}:
\begin{itemize}
...
\glslink{react-component}{Komponent} \texttt{DetailedPostActions} (rys. \ref{img:detailed-post-actions}) wyświetla panel akcji użytkownika dla posta w widoku szczegółowym.
Zawiera:
\begin{itemize}
...
\glslink{react-component}{Komponent} \texttt{PostCommentList} (rys. \ref{img:post-comment-list-1}, \ref{img:post-comment-list-2}) odpowiada za wyświetlanie listy komentarzy przypisanych do posta lub do innego komentarza (w przypadku odpowiedzi).
...
\begin{itemize}
    \item wyświetlanie listy komentarzy przy użyciu \glslink{react-component}{komponentu} \texttt{PostComment} (rys. \ref{img:post-comment-1} - \ref{img:post-comment-3})
...
\begin{figure}[H]
    ...
    \caption{Komponent PostCommentList (1/2)}
...
\begin{figure}[H]
    ...
    \caption{Komponent PostCommentList (2/2)}
    \label{img:post-comment-list-2}
\end{figure}

\glslink{react-component}{Komponent} \texttt{PostComment} (rys. \ref{img:post-comment-1} - \ref{img:post-comment-3}) odpowiada za prezentację pojedynczego komentarza oraz obsługę wszystkich interakcji użytkownika z nim związanych.
...
\begin{figure}[H]
    ...
    \caption{Komponent PostComment (1/3)}
...
\begin{figure}[H]
    ...
    \caption{Komponent PostComment (2/3)}
...
\begin{figure}[H]
    ...
    \caption{Komponent PostComment (3/3)}
