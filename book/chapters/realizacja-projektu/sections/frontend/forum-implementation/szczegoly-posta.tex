...
Komponent \texttt{ForumThread} odpowiada za wyświetlanie pełnej treści pojedynczego posta wraz z komentarzami.
...
\begin{itemize}
    ...
    \item \texttt{PostCommentForm} – formularz dodawania komentarza, otwierany po kliknięciu w przycisk w \texttt{DetailedPost}. Widoczność kontrolowana jest lokalnym stanem komponentu
...
\texttt{DetailedPost} składa się z następujących komponentów:
\begin{itemize}
    \item \texttt{ForumContentHeader} – nagłówek posta z informacjami o autorze i dacie publikacji. Kliknięcie w autora przenosi do jego profilu
    \item \texttt{PostMetaData} – wyświetla kategorię i tagi posta
    \item \texttt{DetailedPostContent} – prezentuje tytuł i treść posta. Treść (\texttt{content}) jest wyświetlana jako HTML, co pozwala na zachowanie formatowania oraz wstawianie elementów multimedialnych
...
Komponent \texttt{DetailedPostActions} wyświetla panel akcji użytkownika dla posta w widoku szczegółowym.
Zawiera:
\begin{itemize}
...
Komponent \texttt{PostCommentList} odpowiada za wyświetlanie listy komentarzy przypisanych do posta lub do innego komentarza (w przypadku odpowiedzi).
Obsługuje zarówno komentarze główne, jak i zagnieżdżone odpowiedzi.

Funkcjonalności komponentu obejmują:
\begin{itemize}
    \item wyświetlanie listy komentarzy przy użyciu komponentu \texttt{PostComment}
    \item obsługę stanów ładowania i błędu
    \item sortowanie komentarzy (tylko dla komentarzy głównych) przy użyciu \texttt{ForumSortDropdown}
...
\begin{itemize}
    \item dla komentarzy głównych – umożliwia sortowanie (np. najnowsze, najstarsze, najlepiej oceniane)
    \item dla odpowiedzi – prezentuje listę bez sortowania i w uproszczonym układzie wizualnym
\end{itemize}

Komponent \texttt{PostComment} odpowiada za prezentację pojedynczego komentarza oraz obsługę wszystkich interakcji użytkownika z nim związanych.
