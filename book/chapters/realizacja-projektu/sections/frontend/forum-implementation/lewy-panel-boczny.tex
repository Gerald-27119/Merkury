...
\glslink{react-component}{Komponent} \texttt{ForumAddPostModal} jest globalnym \glslink{modal}{modalem} tworzonym w ramach \texttt{ForumLayout} i odpowiada za dodawanie oraz edycję postów na forum.
...
\glslink{react-component}{Komponent} \texttt{PostForm} zawiera formularz dodawania i edycji posta (rys. \ref{img:post-form-1}, \ref{img:post-form-2}).

Formularz został zaimplementowany z użyciem:
\begin{itemize}
    \item \texttt{React Hook Form} – do zarządzania stanem formularza
    \item \texttt{Zod} – do walidacji danych wejściowych (schema \texttt{PostFormSchema}) (rys. \ref{img:zod-schema})
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/zod_schema}
    \caption{Przykład użycia Zod schema}
    \label{img:zod-schema}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/post_form_1}
    \caption{Komponent PostForm (1/2)}
    \label{img:post-form-1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{attachments/implementacja-frontendu/forum/post_form_2}
    \caption{Komponent PostForm (2/2)}
    \label{img:post-form-2}
\end{figure}

Pole \texttt{content} wykorzystuje kontrolowany edytor tekstu (\texttt{ControlledEditor}) (rys. \ref{img:controlled-editor}) oparty o \glslink{tiptap}{bibliotekę Tiptap}, działający w wariancie \glslink{modal}{modalnym}.
...
\glslink{react-component}{Komponent} \texttt{ControlledEditor} (rys. \ref{img:controlled-editor}) pełni rolę warstwy integracyjnej pomiędzy React Hook Form a \glslink{rich-text-editor}{edytorem rich text} opartym o \glslink{tiptap}{bibliotekę Tiptap}.
...
Sam \glslink{react-component}{komponent} \texttt{Tiptap} (rys. \ref{img:tiptap-1} - \ref{img:tiptap-3}) został skonfigurowany od podstaw z wykorzystaniem zestawu rozszerzeń, m.in.:
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/implementacja-frontendu/forum/tiptap_1}
    \caption{Komponent Tiptap (1/3)}
    \label{img:tiptap-1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/tiptap_2}
    \caption{Komponent Tiptap (2/3)}
    \label{img:tiptap-2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/tiptap_3}
    \caption{Komponent Tiptap (3/3)}
    \label{img:tiptap-3}
\end{figure}

Edytor posiada własny pasek narzędzi (\texttt{MenuBar}) (rys. \ref{img:menubar-1}, \ref{img:menubar-2}) zsynchronizowany ze stanem edytora, oferujący:
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/menubar_1}
    \caption{Komponent Menubar (1/2)}
    \label{img:menubar-1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{attachments/implementacja-frontendu/forum/menubar_2}
    \caption{Komponent Menubar (2/2)}
    \label{img:menubar-2}
\end{figure}
...
\texttt{PostCategoriesTagsPanel} (rys. \ref{img:post-categories-tags-panel}) jest \glslink{react-component}{komponentem} lewego panelu bocznego forum, odpowiedzialnym za prezentację listy kategorii oraz tagów postów.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/forum/post_categories_tags_panel}
    \caption{Komponent PostCategoriesTagsPanel}
    \label{img:post-categories-tags-panel}
\end{figure}
