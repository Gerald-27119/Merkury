...
Komponent \texttt{PostForm} zawiera formularz dodawania i edycji posta (rys. \ref{img:post-form-1}~\ref{img:post-form-2}).
...
Komponent \texttt{ControlledEditor} (rys. \ref{img:controlled-editor}) pełni rolę warstwy integracyjnej pomiędzy React Hook Form a \glslink{rich-text-editor}{edytorem rich text} opartym o \glslink{tiptap}{bibliotekę Tiptap}.
...
Sam komponent \texttt{Tiptap} (rys. \ref{img:tiptap-1} - \ref{img:tiptap-3}) został skonfigurowany od podstaw z wykorzystaniem zestawu rozszerzeń, m.in.:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Komponent Tiptap (3)}
    \label{img:tiptap-3}
\end{figure}

Edytor posiada własny pasek narzędzi (\texttt{MenuBar}) (rys. \ref{img:menubar-1}~\ref{img:menubar-2}) zsynchronizowany ze stanem edytora, oferujący m.in.:
\begin{itemize}
...
\texttt{PostCategoriesTagsPanel} (rys. \ref{img:post-categories-tags-panel}) jest komponentem lewego panelu bocznego forum, odpowiedzialnym za prezentację listy kategorii oraz tagów postów.
