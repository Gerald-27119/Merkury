...
Do stylowania interfejsu wykorzystaliśmy \gls{framework} Tailwind CSS.
...
użytych \gls{bibliote}k tego wymagała. W innych przypadkach wystarczyło skorzystać z klas
zdefiniowanych w \texttt{index.css} oraz klas Tailwinda.
Cała aplikacja jest \gls{responsywn}a. Tailwind udostępnia predefiniowane prefiksy
responsywne (np. \texttt{md:}, \texttt{lg:}) (rys. \ref{img:tailwind-responsywnosc}), które pozwalają przypisywać style zależnie od
...
Aby uzyskać płynniejsze i przyjemniejsze animacje, wykorzystaliśmy \gls{bibliote}kę Framer Motion.
Pozwala ona w prosty sposób tworzyć animacje elementów interfejsu, bez potrzeby ręcznego pisania
złożonych reguł CSS. W naszej aplikacji użyliśmy jej m.in. w polach formularza logowania i rejestracji
(rys. \ref{img:motion}).
...
\begin{figure}[H]
    ...
    \caption{Implementacja animacji z wykorzystaniem Framer Motion}
    \label{img:motion}
\end{figure}
