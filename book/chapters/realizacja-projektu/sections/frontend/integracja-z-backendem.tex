...
\label{subsec:integracja-i-komunikacji-z-backendem}

Jest to kluczowy element aplikacji, ponieważ wymaga bezpiecznego przesyłania danych użytkownika.
W celu uproszczenia komunikacji z serwerem skorzystaliśmy z biblioteki \texttt{axios} oraz \gls{bibliote}ki \texttt{TanStack Query}.
We wszystkich ścieżkach, które wymagają, aby użytkownik był zalogowany, do zapytania dołączany jest token
\gls{jwt}. Token jest przekazywany w ciasteczku dzięki ustawieniu parametru \texttt{withCredentials} na wartość \texttt{true}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/account-axios1}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/account-axios2}
...
stanów zapytania (takich jak ładowanie danych, błąd, sukces). \gls{bibliote}ka udostępnia m.in.
...
Dodatkowo \gls{hook} \texttt{useQuery} z tej \gls{bibliote}ki umożliwia automatyczne pobieranie danych po wejściu na daną podstronę.
...
wykorzystujemy \gls{hook} \texttt{useMutation} z TanStack Query.
Przykład użycia tego rozwiązania w procesie
logowania został przedstawiony na rys. \ref{img:login-tanstack}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/login-tanstack}
