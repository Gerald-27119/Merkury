...
\end{itemize}
Pozycja submenu renderowana jest jako przycisk zawierający \newline \texttt{SidebarItemContent}, a lista elementów potomnych
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemSubmenu} (1/3).}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemSubmenu} (2/3).}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemSubmenu} (3/3).}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemLink} (1/2).}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemLink} (2/2).}
    \label{img:sidebar-item-link-2}
\end{figure}

\textbf{\texttt{SidebarItemContent}} \\
Komponent stanowi wspólne „wnętrze” elementów \texttt{SidebarItem}.
Zawiera \texttt{SidebarIcon}, warunkowo \texttt{Tooltip} (gdy pasek jest zwinięty i aktywny jest \glslink{stan}{stan} tooltipa)
...
Zaimplementowano akcję wylogowania (wywołanie \texttt{logout} oraz aktualizacja \glslink{stan}{stanu} konta)
oraz przełączanie motywu poprzez \texttt{onChangeTheme}.
Renderowany jest przycisk zawierający \newline \texttt{SidebarItemContent} (rys. \ref{img:sidebar-item-action-1}--\ref{img:sidebar-item-action-2}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-frontendu/sidebar/sidebar-item-action1}
    \caption{Fragment kodu komponentu \texttt{SidebarItemAction} (1/2).}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu komponentu \texttt{SidebarItemAction} (2/2).}
...
\texttt{staticLinks}, \texttt{userLoggedLinks} oraz \newline \texttt{getOptionsLinks} (rys. \ref{img:sidebar-links-1}--\ref{img:sidebar-links-3}).
Zależnie od \glslink{stan}{stanu} zalogowania (\texttt{isLogged}) generowane są różne zestawy pozycji:
\begin{itemize}
...
\newline \texttt{SidebarSubmenuLink}, \texttt{SidebarSubmenu}, \texttt{SidebarAction} (rys. \ref{img:sidebar-types}).
...
Przechowywana jest wyłącznie flaga \texttt{isOpen} informująca o \glslink{stan}{stanie} rozwinięcia.
Zdefiniowano:
\begin{itemize}
...
\newline \texttt{dispatch(sidebarAction.setIsSidebarOpen(true))} (rys. \ref{img:layout}).
