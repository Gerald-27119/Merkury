...
\begin{itemize}
    ...
    od \glslink{serwer}{serwera} do \glslink{klient}{klientów}
...
\begin{figure}[H]
    ...
    \caption{Serwis odpowiedzialny za komunikację za pomocą WebSocket'a.}
...
już istniejącego połączenia \gls{websocket}, bez konieczności implementowania odrębnej logiki połączenia,
subskrypcji i obsługi zdarzeń. Rozwiązanie składa się z:
\begin{itemize}
    \item \texttt{WebSocketService}
    -- zarządza pojedynczym połączeniem \glslink{stomp}{STOMP} działającym nad \glslink{websocket}{WebSocket}:
    nawiązuje i utrzymuje połączenie, obsługuje \gls{reconnect}, rejestruje oraz usuwa \glslink{subskrypcja}{subskrypcje},
    a także wysyła komunikaty do wskazanej \glslink{destynacja}{destynacji},
    \item \texttt{useWebSocket}
    -- udostępnia komponentom prosty interfejs do pracy z połączeniem:
    automatycznie tworzy i usuwa \glslink{subskrypcja}{subskrypcje},
    dostarcza funkcję do wysyłania wiadomości i zwraca status połączenia,
    \item \texttt{WebSocketProvider}
    -- udostępnia współdzieloną instancję serwisu przez \gls{kontekst} i steruje cyklem życia połączenia na poziomie aplikacji
    (łączy po zalogowaniu i rozłącza po wylogowaniu),
    \item \texttt{createChatSubscription}
    -- definiuje \glslink{destynacja}{destynacje} i \glslink{callback}{callback'i} obsługujące odbierane komunikaty,
    aktualizując \gls{stan} w \gls{redux}. Rozwiązanie ma charakter rozszerzalny. Kolejne moduły mogą dostarczać własne fabryki,
    które następnie są dodawane w \texttt{WebSocketProvider} do wspólnej listy subskrypcji, bez konieczności modyfikowania logiki połączenia.
...
\begin{figure}[H]
    ...
    \caption{Serwis odpowiedzialny za komunikację za pomocą WebSocket'a (część 1/4).}
...
\subsubsection{Obsługa subskrypcji w cyklu życia komponentu}
...
\subsubsection{Zarządzanie cyklem życia połączenia}
...
3. nowe zdjecia z wiekszym przyblizeniem
