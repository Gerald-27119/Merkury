...
na potrzeby realizacji modułu czatu.
Opis przygotowano w oparciu o specyfikację \gls{rfc}6455 oraz dokumentację
\gls{spring-framework} dotyczącą \glslink{websocket}{WebSocket} i \glslink{stomp}{STOMP}
...
Na podstawie specyfikacji \gls{rfc}6455, opracowanej przez \gls{ietf}, protokół
\glslink{websocket}{WebSocket} można opisać jako mechanizm komunikacji pomiędzy aplikacją
działającą w przeglądarce a \gls{serwer}, który umożliwia dwukierunkową wymianę danych
...
weryfikując domenę, z której pochodzi kod.
...
połączeniach \gls{http} i okresowym odpytywaniu \glslink{serwer}{serwera}.
...
Nie narzuca natomiast \textbf{struktury komunikatów aplikacyjnych}, sposobu ich adresowania, routingu ani modelu
...
Z tego powodu często stosuje się protokoły warstwy aplikacyjnej działające \textbf{nad}
...
Pierwsze to praca na „surowych” \glslink{ramka}{ramkach} \glslink{websocket}{WebSocket}
(pełna kontrola, ale konieczność zaprojektowania własnego formatu komunikatów i dystrybucji).
...
pozwala korzystać z gotowych \glslink{destynacja}{destynacji} (ścieżek), \glslink{subskrypcja}{subskrypcji}
oraz mechanizmu publikowania wiadomości.

\subsubsection{Plik \texttt{WebSocketConfig.java} --- konfiguracja \glslink{websocket}{WebSocket}}
...
\begin{figure}[H]
    ...
    \caption{Konfiguracja \glslink{websocket}{WebSocket} w module czatu (plik \texttt{WebSocketConfig.java}).}
...
\begin{itemize}
    ...
    \textbf{od \glslink{klient}{klienta} do \glslink{serwer}{serwera}}
    ...
    \textbf{od \glslink{serwer}{serwera} do \glslink{klient}{klientów}}
...
\subsubsection{Plik \texttt{ChatStompCommunicationController.java} --- kontroler \glslink{stomp}{STOMP}}
...
\subsubsection{Plik \texttt{ChatStompCommunicationService.java} --- serwis dystrybucji wiadomości}
...
\begin{itemize}
    ...
    aby \glslink{frontend}{frontend} mógł powiązać wiadomość zapisaną w \gls{baza-danych} z odpowiadającą jej wersją „optymistyczną”.
\end{itemize}

\subsubsection{Zestawienie wykorzystywanych \glslink{destynacja}{destynacji}}
\label{subsubsec:ws-destynacje-backend}

Poniżej zestawiono wszystkie \glslink{destynacja}{destynacje} używane w komunikacji \glslink{websocket}{WebSocket} w module czatu:
...
W \glslink{frontend}{frontendzie} zastosowano podejście oparte na jednej współdzielonej instancji serwisu komunikacji,
udostępnianej poprzez \gls{kontekst}. Rozwiązanie składa się z:
\begin{itemize}
    \item serwisu komunikacyjnego \texttt{WebSocketService} (utrzymanie połączenia, \glslink{subskrypcja}{subskrypcje}, publikacja),
    \item hooka \texttt{useWebSocket} (ergonomiczne użycie w komponentach),
    \item providera \texttt{WebSocketProvider} (utrzymanie cyklu życia połączenia zależnie od logowania),
    \item fabryk subskrypcji (np.\ \texttt{createChatSubscription}) aktualizujących \gls{stan} w \gls{redux}.
\end{itemize}

\subsubsection{Plik \texttt{WebSocketService.ts} --- klasa \texttt{WebSocketService}}
...
\begin{figure}[H]
    ...
    \caption{Klasa \texttt{WebSocketService} w \gls{type-script} (część 1/4).}
...
\begin{figure}[H]
    ...
    \caption{Klasa \texttt{WebSocketService} w \gls{type-script} (część 2/4).}
...
\begin{figure}[H]
    ...
    \caption{Klasa \texttt{WebSocketService} w \gls{type-script} (część 3/4).}
...
\begin{figure}[H]
    ...
    \caption{Klasa \texttt{WebSocketService} w \gls{type-script} (część 4/4).}
...
\glslink{stomp}{STOMP} działającym nad \glslink{websocket}{WebSocket}. Do zestawienia transportu wykorzystano \gls{sockjs},
...
Dane \texttt{payload} są poddawane \gls{serializacja} do formatu \gls{json}, a następnie wysyłane do brokera.
Opcjonalnie dołączane są \glslink{naglowek}{nagłówki} protokołu \glslink{stomp}{STOMP}.

\subsubsection{Plik \texttt{useWebSocket.ts} --- hook \texttt{useWebSocket}}
\label{subsubsec:front-use-websocket}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-websocket/front/infra/use_1}
    \caption{Hook \texttt{useWebSocket} w \gls{type-script} (część 1/2).}
...
\begin{figure}[H]
    ...
    \caption{Hook \texttt{useWebSocket} w \gls{type-script} (część 2/2).}
    \label{img:web-socket-use-2}
\end{figure}

Hook \texttt{useWebSocket} upraszcza korzystanie z komunikacji \glslink{websocket}{WebSocket}/\glslink{stomp}{STOMP}
w \gls{react}. Zapewnia automatyczne zakładanie i zdejmowanie \glslink{subskrypcja}{subskrypcji} zgodnie z
\gls{cykl_zycia} komponentu oraz udostępnia funkcję do wysyłania wiadomości.
...
\begin{itemize}
    ...
    \item w \gls{use_effect} zakłada subskrypcje podczas \gls{montowanie} komponentu oraz usuwa je podczas \gls{odmontowanie}
    (lub przy zmianie zależności).
\end{itemize}

\textbf{Ważna uwaga o \gls{tablica_zaleznosci}:}
w implementacji śledzone są tylko same \texttt{destination}. Oznacza to, że zmiana samej funkcji \texttt{callback}
bez zmiany destynacji nie wymusi ponownego zasubskrybowania (jest to zwykle akceptowalne, ale należy o tym pamiętać).

\subsubsection{Plik \texttt{WebSocketContext.tsx} --- \gls{kontekst} i provider połączenia}
\label{subsubsec:front-ws-context}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{attachments/implementacja-websocket/front/infra/context}
    \caption{Kontekst \texttt{WebSocketContext} oraz komponent \texttt{WebSocketProvider}.}
    \label{img:web-socket-context}
\end{figure}

Plik definiuje \gls{kontekst} React, który udostępnia w całej aplikacji jedną wspólną instancję
\texttt{WebSocketService}. Dzięki temu wiele komponentów może korzystać z jednego połączenia
\glslink{websocket}{WebSocket}/\glslink{stomp}{STOMP}, zamiast tworzyć osobne połączenia niezależnie.
...
\begin{itemize}
    ...
    \item podczas sprzątania wykonywany jest \gls{cleanup} subskrypcji, aby uniknąć „wiszących” kanałów.
\end{itemize}

Dodatkowo używany jest \gls{redux_dispatch} (funkcja \texttt{dispatch}), aby aktualizować \gls{stan} aplikacji w \gls{redux}
w reakcji na wiadomości przychodzące w czasie rzeczywistym.

\subsubsection{Plik \texttt{ChatSubscription.ts} --- fabryka subskrypcji czatu}
...
\begin{figure}[H]
    ...
    \caption{Funkcja \texttt{createChatSubscription} tworząca definicję subskrypcji czatu.}
...
Jej celem jest odbieranie nowych wiadomości w czasie rzeczywistym oraz aktualizacja \gls{stan}u aplikacji w \gls{redux}.
...
\begin{itemize}
    ...
    \item w \gls{callback} wykonuje \gls{deserializacja} wiadomości z \gls{json} (\texttt{JSON.parse}),
    \item wysyła \gls{akcja_redux} do \gls{redux} w celu aktualizacji „ostatniej wiadomości” w danym czacie,
...
wynika z asynchronicznego charakteru odbioru wiadomości: callback wykonuje się w chwili nadejścia komunikatu,
dlatego odczyt bieżącego stanu bezpośrednio ze \gls{store} minimalizuje ryzyko użycia nieaktualnych danych
(np.\ przy szybkim przełączaniu czatów przez użytkownika).
