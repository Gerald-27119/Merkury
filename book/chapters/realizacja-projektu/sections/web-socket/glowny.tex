...
% to odniesienie do RFC bez glslink jest celowe
...
np. podczas logowania użytkownika, pobierania listy \glslink{spot}{spotów} czy dodawania komentarza na forum.
...
\begin{itemize}
    \item \texttt{/connect} -- \gls{endpoint} ustanowienia połączenia (tzw.\ \gls{handshake}); \texttt{withSockJS()} zapewnia mechanizm \gls{fallback} (emulację WebSocket po HTTP), gdy \gls{websocket} jest blokowany.
    ...
    (\gls{backend} rozpoznaje, kto wysłał wiadomość).
\end{itemize}
...
Po zapisaniu wiadomości \glslink{serwer}{serwer} rozsyła ją do wszystkich uczestników czatu wraz z identyfikatorem
nadanym w \glslink{baza-danych}{bazie danych}, co pozwala \glslink{frontend}{frontendowi} powiązać wiadomość utrwaloną
z jej wersją \glslink{optimistic-ui}{optymistyczną}.
...
\begin{itemize}
    ...
    \item \texttt{\gls{hook} useWebSocket}
    -- zapewnia warstwę użycia w komponentach: automatycznie rejestruje i usuwa \glslink{subskrypcja}{subskrypcje}
    zgodnie z \glslink{cykl_zycia}{cyklem życia komponentu} oraz udostępnia funkcję wysyłania wiadomości i status połączenia,
...
Dane \texttt{payload} są poddawane \glslink{serializacja}{serializacji} do formatu \gls{json}, a następnie wysyłane do \glslink{backend}{backendu}.
...
\glslink{hook}{Hook} \texttt{useWebSocket} upraszcza korzystanie z komunikacji \glslink{websocket}{WebSocket}
...
\begin{itemize}
    \item pobiera instancję serwisu z \texttt{useWebSocketService()} (z \glslink{kontekst}{kontekstu}),
...
\newline \texttt{WebSocketService}. Dzięki temu wiele komponentów może korzystać z jednego połączenia
\glslink{websocket}{WebSocket}, zamiast tworzyć osobne połączenia niezależnie.
\newline
\textbf{Adres endpointu:}
\texttt{WS\_URL} jest pobierany ze \glslink{zmienna_srodowiskowa}{zmiennej środowiskowej} \newline \texttt{REACT\_APP\_WS\_URL},
a w razie braku ustawienia przyjmuje wartość domyślną \newline \texttt{http://localhost:8080/connect}.

\textbf{Współdzielona instancja serwisu:}
\texttt{wsService} jest tworzony raz (wzorzec \gls{singleton}) poza komponentem providera.
Zapewnia to, że aplikacja nie tworzy nowego połączenia przy każdym renderowaniu.
...
\begin{itemize}
    ...
    \item po zalogowaniu rejestrowane są \glslink{subskrypcja}{subskrypcje} zależne od użytkownika (np.\ kanał czatów),
    \item podczas sprzątania wykonywany jest \gls{cleanup} \glslink{subskrypcja}{subskrypcji}.
\end{itemize}

Dodatkowo używany jest \gls{redux_dispatch}, aby aktualizować \gls{stan} aplikacji w \glslink{redux}{Redux'sie},
...
Jej celem jest odbieranie nowych wiadomości w czasie rzeczywistym oraz aktualizacja \glslink{stan}{stanu} aplikacji w \glslink{redux}{Redux'sie}.
...
dlatego odczyt bieżącego \glslink{stan}{stanu} bezpośrednio ze \gls{store} minimalizuje ryzyko użycia nieaktualnych danych.
