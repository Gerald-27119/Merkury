...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item w praktyce występuje wiele klas tego typu (np.\ \texttt{ConcreteHandlerA}, \texttt{ConcreteHandlerB}, \texttt{ConcreteHandlerC}),
...
\begin{itemize}
    ...
    \item \textbf{odpowiednik przekazania do \texttt{next}}: wywołanie \texttt{filterChain.doFilter(request, response)}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Pobranie danych wejściowych} -- odczyt tokenu \glslink{jwt}{JWT} z żądania
    (np.\ z \glslink{http-only-cookie}{ciasteczka \texttt{HttpOnly}}).
    \item \textbf{Walidacja} -- sprawdzenie poprawności tokenu \glslink{jwt}{JWT}. W przypadku błędu filtr przygotowuje odpowiedź
    ...
    \item \textbf{Odświeżenie tokenu (opcjonalnie)} -- gdy czas ważności tokenu \glslink{jwt}{JWT} zbliża się do końca, generowany jest nowy token.
