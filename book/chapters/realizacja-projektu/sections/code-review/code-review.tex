...
W projekcie do każdego zadania była tworzona dedykowana gałąź (ang. \emph{branch}), którą po jego wykonaniu
\glslink{merge}{mergowano} do gałęzi głównej.
Aby proces ten był możliwy, tworzono \glslink{pull-request}{pull requesty}, czyli mechanizm pozwalający innym członkom
zespołu na przeczytanie kodu oraz zgłaszanie ewentualnych uwag lub pytań.
Umożliwiały one również inicjalizację procesów \glslink{cicd}{CI/CD} (por. sekcja~\ref{sec:implementacja-ci-cd}), które między innymi
uruchamiały testy.
Uwagę można było zamieścić do całego pliku, jego konkretnych linii lub ogólnie (bez powiązania z plikiem).
...
Mając na uwadze, że modele sztucznej inteligencji mogą popełniać błędy, każda uwaga \texttt{Copilot'a} była weryfikowana
w innych źródłach.
Warto zaznaczyć, że mimo tego narzędzie to pozwalało skorygować nieprawidłowości, które członkowie zespołu
przeoczyli.
\texttt{Copilot} mając dostęp do szerokiej bazy wiedzy, proponował rozwiązania o których zespół nie wiedział ze względu
na między innymi ograniczone doświadczenie.

Po rozwiązaniu wszystkich uwag, \glslink{pull-request}{pull request} otrzymywał zatwierdzenie wprowadzonych zmian
(ang. approval), co umożliwiało \glslink{merge}{zmergowanie} go do głównej gałęzi.
