...
\begin{figure}[H]
    ...
    \caption{Diagram przedstawiający przepły OAuth 2.0; źródło: \cite{oauth-docs-rfc}}
...
\begin{itemize}
    ...
    \item \texttt{Resource Owner} \textendash \space właściciel zasobu, do którego dostęp prosi aplikacja; w tym wypadku
...
\begin{enumerate}[label=\Alph*.]
    ...
    \item \texttt{Client'owi} zwracane jest potwierdzenie przyznania dostępu, na przykład w formie \glslink{token}{tokenu}.
    \item \texttt{Client} przesyła potwierdzenie dostępu do \texttt{Authorization Server'a} uwierzytelniając się i prosząc
    o \glslink{token}{token} dostępu do zasobu z \texttt{Resource Server'a}.
    \item \texttt{Authorization Server} sprawdza przesłane dane i jeśli proces walidacji zostanie zakończony pomyślnie,
    przekazuje \glslink{token}{token} dostępu do zasobu.
    \item \texttt{Client} wysyła zapytanie z uzyskanym wcześniej \glslink{token}{tokenem} do \texttt{Resource Server'a} prosząc o przesłanie
    zasobu.
    \item \texttt{Resource Server} przeprowadza walidację \glslink{token}{tokena} i jeśli zakończy się ona powodzeniem, przesyła zasób
    \texttt{Client'owi}.
\end{enumerate}

W projekcie do implementacji \glslink{oauth}{OAutha 2.0} wykorzystano \glslink{biblioteka}{biblioteki} \texttt{Spring Boot}:
\newline \texttt{spring-boot-starter-oauth2-resource-server} oraz \\ \texttt{spring-boot-starter-oauth2-client}.
...
utworzenie konta deweloperskiego i podanie linków, na które użytkownicy po pomyślnym procesie będą przekierowywani.
Od każdego z nich otrzymano parę: \texttt{CLIENT\_ID}, \texttt{CLIENT\_SECRET}, które pozwalają na uwierzytelnienie aplikacji.
Dane te nie powinny być dostępne dla osób trzecich, dlatego ze względów bezpieczeństwa umieszczono je w zmiennych
...
Z \glslink{token}{tokenu} przesłanego przez \texttt{Provider'a} ustalany jest jego typ (Google lub \glslink{github}{GitHub}) oraz
obiekt zakodowanego użytkownika.
Następnie odczytywane są jego nazwa i adres email, w przypadku niepowodzenia rzucany jest odpowiedni wyjątek.
Po sprawdzeniu czy użytkownik istnieje w bazie danych, wywoływane są odpowiednio metody do logowania lub
rejestracji.
