...
\eTOecard{tab:e2e:acc01}{Logowanie użytkownika}{%
    ...
    \eTOepre{Konto istnieje w sytemie.}
    \eTOesteps{%
...
\eTOecard{tab:e2e:add01}{Wyświetlenie stanu pustego dla dodanych \newline spotów}{%
    \eTOeid{E2E-ADD-01}
    \eTOegoal{Weryfikacja poprawnej obsługi pustej listy dodanych spotów.}
    ...
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            ...
            \item Oczekiwanie na odpowiedź endpointu z listą spotów (pusta lista).
        \end{tabitemizeeTOe}
    }
    \eTOeexpected{Wyświetlenie komunikatu o braku dodanych spotów.}
}
...
\eTOecard{tab:e2e:add02}{Ładowanie kolejnych elementów listy przy przewijaniu (\textit{infinite scroll})}{%
    ...
    \eTOetype{Mockowane API}
    \eTOepre{Użytkownik jest zalogowany i znajduje się na widoku listy z włączonym nieskończonym przewijaniem; w źródle danych istnieją kolejne elementy do załadowania.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            \item Otwarcie widoku \textit{Add spot}.
...
\eTOecard{tab:e2e:add03}{Otwarcie modala dodawania spota na widoku desktop}{%
    \eTOeid{E2E-ADD-03}
    \eTOegoal{Weryfikacja dostępności modala dodawania spota w trybie desktop.}
    \eTOetype{Mockowane API}
    \eTOepre{Zasymulowanie zalogowania poprzez wpisy w \textit{localStorage}.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
    ...
    \eTOeexpected{Wyświetlenie modala dodawania spota wraz z sekcjami (\textit{Basic Information}, \textit{Upload Media}).}
}

\eTOecard{tab:e2e:add04}{Zablokowanie dodawania spota na małym \newline ekranie}{%
    \eTOeid{E2E-ADD-04}
    \eTOegoal{Weryfikacja blokady dodawania spota dla małych rozdzielczości.}
    \eTOetype{Mockowane API}
    \eTOepre{Zasymulowanie zalogowania poprzez wpisy w \textit{localStorage}.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            ...
            \item Otwarcie widoku \textit{Add spot}.
            \item Próba otwarcia modala dodawania spota.
        \end{tabitemizeeTOe}
    }
    \eTOeexpected{Wyświetlenie komunikatu o wymaganym większym ekranie oraz brak wyświetlenia formularza dodawania.}
}

\eTOecard{tab:e2e:add05}{Dodanie nowego spota z użyciem rzeczywistego backendu}{%
    \eTOeid{E2E-ADD-05}
    \eTOegoal{Weryfikacja pełnego procesu dodania spota (UI $\rightarrow$ backend).}
    \eTOetype{Rzeczywisty backend}
    ...
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            ...
            \item Otwarcie modala dodawania spota.
            ...
            \item Zatwierdzenie dodania spota.
        \end{tabitemizeeTOe}
    }
    \eTOeexpected{Zapis spota, komunikat o powodzeniu oraz obecność nowo dodanego spota na liście.}
}
...
\eTOecard{tab:e2e:com03}{Załadowanie widoku komentarzy po Rzeczywistym logowaniu}{%
    ...
    \eTOetype{Rzeczywisty backend}
    \eTOepre{Użytkownik posiada konto i może się na nie zalogować.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            \item Wykonanie logowania użytkownika.
...
\eTOecard{tab:e2e:fav01}{Wyświetlenie stanu pustego dla list ulubionych spotów}{%
    \eTOeid{E2E-FAV-01}
    \eTOegoal{Weryfikacja obsługi pustej listy ulubionych spotów.}
    \eTOetype{Mockowane API}
    \eTOepre{Zasymulowanie zalogowania poprzez wpisy w \textit{localStorage}.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            \item Otwarcie widoku ulubionych spotów.
            \item Pobranie pustej listy.
        \end{tabitemizeeTOe}
    }
    \eTOeexpected{Komunikat o braku spotów w liście.}
}
...
\eTOecard{tab:e2e:fav03}{Dociąganie kolejnej strony przy przewijaniu}{%
    \eTOeid{E2E-FAV-03}
    \eTOegoal{Weryfikacja infinite scroll w liście ulubionych spotów.}
    \eTOetype{Mockowane API}
    \eTOepre{Zasymulowanie zalogowania poprzez wpisy w \textit{localStorage}.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
...
\eTOecard{tab:e2e:fav04}{Załadowanie danych z rzeczywistego backendu po logowaniu}{%
    \eTOeid{E2E-FAV-04}
    \eTOegoal{Weryfikacja pobrania danych ulubionych spotów z backendu po logowaniu.}
    \eTOetype{Rzeczywisty backend}
    \eTOepre{Użytkownik posiada konto i może się na nie zalogować.}
    \eTOesteps{%
        \begin{tabitemizeeTOe}
            \item Logowanie użytkownika.
            \item Przejście do widoku ulubionych spotów.
...
\eTOecard{tab:e2e:pro03}{Nawigacja do listy znajomych z poziomu \newline profilu}{%
    \eTOeid{E2E-PRO-03}
