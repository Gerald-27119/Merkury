...
\begin{itemize}
    ...
    a także dodanie do projektu potrzebnych \glslink{biblioteka}{bibliotek}
    m.in.: \gls{tanstack-query}, \gls{redux} oraz \gls{tailwind-css}.

    \item \textbf{Uwierzytelnianie i bezpieczeństwo} --
    implementacja logowania i rejestracji, doprecyzowanie ról i uprawnień
    w \glslink{spring-security}{Spring Security},
    a także logowanie za pomocą \glslink{oauth}{OAuth} poprzez Google
    oraz \glslink{github}{GitHub}.
    ...
    \item \textbf{Środowisko uruchomieniowe} --
    konteneryzacja z użyciem \glslink{docker}{Dockera}
    (m.in. \gls{docker-compose} oraz pliki \gls{dockerfile})
    oraz przygotowanie skryptów ułatwiających uruchamianie
    środowiska deweloperskiego.

    \item \textbf{Jakość i automatyzacja} --
    rozwój testów (\glslink{integration-tests}{integracyjnych},
    \glslink{unit-tests}{jednostkowych} i \glslink{e2e-tests}{E2E}
    tam, gdzie zasadne),
    uruchamianie testów w potokach \gls{cicd},
    ujednolicenie formatowania (m.in. \gls{prettier})
    i kontrola jakości w repozytorium (np. \gls{eslint}).

    \item \textbf{Wydajność} --
    wprowadzenie mechanizmu optymalizacji w postaci \glslink{cache}{cache'a}
    (np. z wykorzystaniem \gls{redis}).

    \item \textbf{Dane deweloperskie} --
    przygotowanie danych testowych wykorzystywanych do demonstracji
    i weryfikacji działania produktu (np. przykładowe \glslink{spot}{spoty}).

    \item \textbf{Komunikaty systemowe} --
    implementacja mechanizmu prezentacji informacji o błędach i sukcesach
    w \glslink{ui}{interfejsie użytkownika}.

    \item \textbf{Dokumentacja} --
    opracowanie dokumentacji projektu w \gls{latex}.
\end{itemize}

Przed rozpoczęciem implementacji modułów przygotowano projekt
\glslink{ui}{interfejsu użytkownika}, obejmujący kluczowe widoki.
Stanowił on punkt odniesienia dla dalszych prac i ułatwił iteracyjne
dopasowywanie funkcjonalności do sposobu użycia systemu.
...
\begin{itemize}
    ...
    przejście na \gls{react-maplibre} oraz ponowna implementacja podstaw
    wyświetlania \glslink{spot}{spotów}.

    \item \textbf{Model \glslink{spot}{spota}} --
    rozszerzanie modelu o dane lokalizacyjne oraz ujednolicenie sposobu
    wyznaczania punktu reprezentującego \glslink{spot}{spota}.
    ...
    \item \textbf{\glslink{responsywnosc}{Responsywność}} --
...
\begin{itemize}
    ...
    wdrożenie \gls{websocket} do pobierania i wysyłania wiadomości.

    \item \textbf{Usprawnienia UX} --
    grupowanie wiadomości po dacie, wielowierszowe pole tekstowe,
    dodanie mechanizmu \glslink{infinite-scroll}{nieskończonego przewijania}
    wiadomości w danym czacie.

    \item \textbf{Emoji i GIF} --
    dodanie obsługi \gls{emoji} oraz wysyłania animowanych obrazów
    \glslink{gif}{GIF}.

    \item \textbf{Wydajność i niezawodność} --
    optymistyczne wysyłanie wiadomości oraz implementacja
    \glslink{paginacja}{paginacji} dla pobierania starszych wiadomości.
...
\begin{itemize}
    ...
    integracja \glslink{rich-text-editor}{edytora rich text}
    na potrzeby tworzenia postów.

    \item \textbf{Media w postach} --
    integracja z \gls{azure-blob-storage} do przesyłania
...
\begin{itemize}
    \item \textbf{Wyszukiwanie \glslink{spot}{spotów}} --
    ...
    \item \textbf{Najpopularniejsze \glslink{spot}{spoty}} --
    dodanie karuzeli z najpopularniejszymi \glslink{spot}{spotami}.

    \item \textbf{Funkcje zaawansowane} --
    \glslink{paginacja}{paginacja} wyników, sortowanie (po popularności i ocenie)
...
\begin{itemize}
    ...
    funkcja dodawania znajomych z \glslink{paginacja}{paginacją}
    i wyszukiwaniem po nazwie użytkownika.

    \item \textbf{Aktywność} --
    sekcje zdjęć i filmów, widok dodanych komentarzy do \glslink{spot}{spota}.

    \item \textbf{Ustawienia konta} --
    edycja danych konta (nazwa użytkownika, e-mail, hasło)
    oraz obsługa zmiany zdjęcia profilowego.

    \item \textbf{Moje \glslink{spot}{spoty} i ulubione} --
