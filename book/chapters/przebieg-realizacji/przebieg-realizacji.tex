...
\begin{itemize}
    ...
    dla \gls{backend} i \gls{frontend}.%glslink z odmianą

    \item \textbf{Podstawy warstwy klienckiej} --
    stworzenie fundamentów aplikacji \glslink{frontend}{frontendowej}
    (routing, konfiguracja stylów, struktury projektu),
    a także dodanie do projektu potrzebnych bibliotek m.in.: \gls{tanstack-query}, \gls{redux} oraz \gls{tailwind-css}.

    \item \textbf{Uwierzytelnianie i bezpieczeństwo} --
    implementacja logowania i rejestracji, doprecyzowanie ról i uprawnień
    w \gls{spring-security}, a także logowanie za pomocą \gls{oauth} poprzez Google oraz \gls{github}.
    ...
    konteneryzacja serwisów, na które składa się aplikacja % do slownika poejcie konteneryzacji
    oraz przygotowanie skryptów ułatwiających uruchamianie środowiska deweloperskiego.

    \item \textbf{Jakość i automatyzacja} --
    rozwój testów (integracyjnych, jednostkowych i E2E tam, gdzie zasadne),
...
\begin{itemize}
    ...
    dodanie obsługi emoji oraz wysyłania animowanych obrazów
    \glslink{gif}{GIF}.
...
\begin{itemize}
    ...
    obsługa postów, kategorii, tagów oraz paginacji.

    \item \textbf{Edytor treści} --
    integracja edytora rich-text na potrzeby tworzenia postów. % dodac odniesienie do glosarisuza
...
\begin{itemize}
    ...
    paginacja wyników, sortowanie (po popularność i ocenie)
