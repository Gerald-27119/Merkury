...
\begin{longtable}{|
    ...
    @ferrucc-io/emoji-picker & Komponent umożliwiający wybór emotikon; udostępnia interfejs wyboru emoji w aplikacji. \\ \hline
    @hookform/resolvers & Adaptery walidatorów dla \texttt{react-hook-form} — ułatwia integrację z bibliotekami walidacji (np. Zod, Yup). \\ \hline
    @reduxjs/toolkit & Oficjalne narzędzie do upraszczania konfiguracji i używania Redux: reducery, slice’y, middleware i helpery. \\ \hline
    @stomp/stompjs & Klient protokołu STOMP do komunikacji poprzez WebSocket — obsługa sesji, subskrypcji i wymiany komunikatów. \\ \hline
    @tailwindcss/vite & Wtyczka integrująca Tailwind CSS z bundlerem Vite — ułatwia kompilację i użycie klasy narzędziowych Tailwind. \\ \hline
    ...
    @tiptap/extension-placeholder & Rozszerzenie Tiptap dodające placeholder (tekst pomocniczy) w pustych polach edytora. \\ \hline
    @tiptap/extension-text-align & Rozszerzenie Tiptap umożliwiające wyrównywanie tekstu (lewy, środek, prawy). \\ \hline
    @tiptap/pm & Silnik edytora (ProseMirror) używany wewnętrznie przez Tiptap — obsługa modelu dokumentu i transformacji. \\ \hline
    @tiptap/react & Integracja edytora Tiptap z Reactem — komponenty i hooki do osadzania edytora w aplikacji. \\ \hline
    @tiptap/starter-kit & Zestaw podstawowych rozszerzeń i konfiguracji dla szybkiego startu z Tiptap. \\ \hline
    @vis.gl/react-maplibre & Wiązania React do MapLibre — komponenty i helpery do osadzania map w aplikacji React. \\ \hline
    antd & Biblioteka komponentów UI dla React (gotowe komponenty: formularze, przyciski, tabele itp.) o ustandaryzowanym wyglądzie. \\ \hline
    axios & Obiektowy klient HTTP do wykonywania zapytań (GET, POST itd.) oraz obsługi odpowiedzi i błędów. \\ \hline
    date-fns & Zestaw funkcji do manipulacji i formatowania dat — lekka alternatywa dla bibliotek typu Moment. \\ \hline
    dotenv & Ładowanie zmiennych środowiskowych z pliku \texttt{.env} — ułatwia konfigurację środowiska. \\ \hline
    maplibre-gl & Silnik renderowania map (WebGL) — warstwy map, kafelki i interakcje geograficzne. \\ \hline
    media-chrome & Zestaw elementów UI do obsługi odtwarzania multimediów w przeglądarce. \\ \hline
    motion & Biblioteka/dostawca narzędzi do animacji interfejsu użytkownika (efekty przejść i animacje komponentów). \\ \hline
    openmeteo & Klient lub wrapper API serwisu pogodowego Open-Meteo — pobieranie prognoz i danych meteorologicznych. \\ \hline
    query-string & Narzędzia do parsowania i generowania parametrów zapytań w URL (query params). \\ \hline
    ...
    react-hook-form & Lekka biblioteka do obsługi formularzy w React: zarządzanie stanem, walidacje i wydajność. \\ \hline
    react-icons & Zbiór ikon udostępnionych jako komponenty React — łatwe osadzanie ikon w UI. \\ \hline
    react-intersection-observer & Hook i API do obserwowania wejścia/wyjścia elementów z pola widzenia (lazy-loading, animacje). \\ \hline
    react-paginate & Komponent pomocniczy do paginacji list i tabel w aplikacji React. \\ \hline
    react-player & Uniwersalny komponent odtwarzacza multimediów wspierający wiele źródeł (YouTube, pliki lokalne itd.). \\ \hline
    react-redux & Oficjalne wiązania umożliwiające użycie store Redux w aplikacji React. \\ \hline
    react-router-dom & Biblioteka do routingu w aplikacjach React: definicja ścieżek, nawigacja i parametry URL. \\ \hline
    react-select & Rozszerzony komponent \texttt{select} z opcjami wyszukiwania, grupowania i stylizacji. \\ \hline
    sockjs-client & Klient WebSocket z fallbackami (SockJS) — stabilniejsza komunikacja w różnych środowiskach sieciowych. \\ \hline
    tailwindcss & Framework CSS typu utility-first — definiowanie wyglądu przy pomocy klas narzędziowych. \\ \hline
    uuid & Generator unikalnych identyfikatorów (UUID) wykorzystywany m.in. do kluczy komponentów i ID obiektów. \\ \hline
    victory & Biblioteka do tworzenia wykresów i wizualizacji danych w React. \\ \hline
    victory-chart & Moduł biblioteki Victory zawierający komponenty i narzędzia do rysowania wykresów. \\ \hline
    zod & Biblioteka do deklaratywnej walidacji schematów danych i ich typowania w TypeScript. \\ \hline
    @testing-library/jest-dom & Rozszerzenia matcherów dla Jesta ułatwiające asercje DOM w testach. \\ \hline
    @testing-library/react & Narzędzia do testowania komponentów React w środowisku DOM. \\ \hline
    @testing-library/user-event & Symulacja zdarzeń użytkownika (kliknięcia, wpisywanie) w testach integracyjnych. \\ \hline
    ...
    @typescript-eslint/eslint-plugin & Zestaw reguł ESLint specyficznych dla TypeScript i praktyk TS. \\ \hline
    ...
    cypress & Narzędzie do end-to-end testów przeglądarkowych — automatyzacja testów funkcjonalnych. \\ \hline
    eslint & Narzędzie do analizy statycznej kodu i egzekwowania reguł stylu/bezpieczeństwa. \\ \hline
    eslint-plugin-react & Wtyczka ESLint z regułami dedykowanymi dla aplikacji React. \\ \hline
    eslint-plugin-react-hooks & Reguły ESLint dotyczące poprawnego użycia hooków React. \\ \hline
    eslint-plugin-react-refresh & Wtyczka wspomagająca integrację z React Fast Refresh podczas developmentu. \\ \hline
    jest & Framework do uruchamiania testów jednostkowych i asercji JavaScript. \\ \hline
    ...
    prettier & Formatowanie kodu automatyzujące styl i ujednolicające składnię. \\ \hline
    prettier-plugin-tailwindcss & Wtyczka Prettiera sortująca klasy Tailwind CSS zgodnie z konwencją. \\ \hline
    tailwind-scrollbar & Plugin Tailwind CSS dodający klasy pomocnicze do stylizacji pasków przewijania. \\ \hline
    typescript & Nadzbiór JavaScript z systemem typów statycznych — kompilacja do JS. \\ \hline
    vite & Szybki bundler i serwer deweloperski zoptymalizowany pod nowoczesne aplikacje webowe. \\ \hline
    vitest & Lekki runner testowy kompatybilny z Vite — szybkie testy jednostkowe i integracyjne. \\ \hline
