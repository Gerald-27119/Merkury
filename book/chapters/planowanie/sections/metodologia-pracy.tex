...
\begin{itemize}
    \item klasyczny Agile (w odmianie Scrum),
...
Ze względu na nierównomierną dostępność zasobów w kolejnych miesiącach studiów nie zapewniono możliwości utrzymania stałej długości sprintów, dlatego z podejścia zrezygnowano.
...
\subsection{Wybrane podejście: Disciplined Agile Delivery}
\label{subsec:wybrane-podejscie-dad-lean}

Podjęto decyzję o zastosowaniu \textbf{Disciplined Agile Delivery}~\cite{DAD} w wariancie \textbf{Lean Life Cycle}~\cite{DAD-LLF}, ponieważ podejście to łączy pożądane cechy Agile i Waterfall, a jednocześnie eliminuje stałe sprinty na rzecz pracy w ciągłym przepływie.
...
\begin{itemize}
    ...
    \item \textbf{Koncentracja na wartości.} Priorytetyzacja wartości biznesowej, wizualizacja pracy, małe partie dostaw.
    \item \textbf{Lekkość i kamienie milowe.} Zastosowano lekkie mechanizmy nadzoru (peer review, prezentacje postępów) zapewniające przejrzystość bez nadmiernej biurokracji.
\end{itemize}

\subsection{Narzędzia i komunikacja}
\label{subsec:narzedzia-komunikacja}

Do zarządzania zadaniami zastosowana została \textbf{Jira} (monitorowanie postępu prac oraz ewidencja zadań członków zespołu).
Komunikację w zespole zaplanowano w formie regularnych spotkań oraz asynchronicznie z wykorzystaniem \textbf{Discorda}
oraz \textbf{Messengera}.

\subsection{Podział ról w zespole}
\label{subsec:podzial-rol}

\begin{itemize}
    \item \textbf{Adam - fullstack developer, lider zespołu}
    \item \textbf{Stanisław - fullstack developer}
    \item \textbf{Kacper - fullstack developer}
    \item \textbf{Mateusz - fullstack developer}
\end{itemize}

Każdy z członków zespołu uczestniczył również w przygotowaniu dokumentacji.
