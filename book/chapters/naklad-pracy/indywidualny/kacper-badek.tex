...
Na pracę nad projektem poświęciłem łącznie 393 godziny, w tym:
\begin{itemize}
    \item 29 godz. \textendash \space zaprojektowanie interfejsu użytkownika modułu forum w figmie
    \item 264 godz. \textendash \space prace deweloperskie
    \item 70 godz. \textendash \space tworzenie dokumentacji
    \item 30 godz. \textendash \space przeprowadzanie \glslink{review-kodu}{review kodu}
\end{itemize}
...
Następnie zaimplementowałem mechanizm resetowania hasła, obejmujący generowanie i weryfikację \glslink{token}{tokenów} oraz ich cykliczne usuwanie.
...
W ramach modułu mapy przygotowałem dane deweloperskie w postaci przykładowych \glslink{spot}{spotów} oraz zaimplementowałem możliwość dodawania ich do ulubionych i przeglądania zapisanych lokalizacji,
a także \glslink{backend}{backend} i \glslink{frontend}{frontend} komentarzy do \glslink{spot}{spotów} z możliwością głosowania.
...
Zrealizowałem obsługę postów i komentarzy (\glslink{crud}{CRUD}), funkcję odpowiadania na komentarze, sortowanie i paginację, edycję treści z wykorzystaniem \glslink{rich-text-editor}{edytora rich text} \glslink{tiptap}{Tiptap}, system głosowania, zgłaszania treści, obserwowania postów oraz wyszukiwarkę z filtrami.
